...
1. **卡牌類型實作（素材到位後）** — 秘術/瞬咒/靈裝/伏印的資料層已預留（`CardData.card_type`，[§7](#7-卡牌類型)），Harvey 之後給素材；屆時實作各型結算與反制窗口（[§5.1](#51-攻擊反制流程每次宣告-6-步)），潛行/沉默關鍵字一併（它們只對法術有意義）。
2. **連線對戰（host-client，見 ADR-001；下一步從 2a 開始）** — 規則側（雙方帳、回合歸屬、牌堆權威）已由熱座完成，剩網路層，拆五小步、每步可獨立驗收：
   - **2a. 大廳與連線骨架** — 主選單加「連線對戰」入口 → 大廳場景（開房：顯示本機 IP 等待加入／加入：輸 IP+Connect）；`ENetMultiplayerPeer` 建線、雙方就緒握手後進 `main.tscn`；host 固定執 `player` 側、client 執 `enemy` 側。
