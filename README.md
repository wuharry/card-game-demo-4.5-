...
- **操作**：把任意手牌（含秘術、瞬咒）拖到自己的**魔力回收區**放開＝棄牌回魔；魔力不足也能拿起並回收。懸停顯示收益或冷卻原因。帳在 `battle_manager.gd` 的 `apply_discard_for_mana()`，UX 與同步在 `card_manager.gd` 的 `_try_discard` / `_net_discard`。墓地僅顯示棄牌；F8 沙盒的直接捨棄不回魔。
...
秘術操作（2026-09-12）：按住手牌拿起，放開才確認；需要目標的秘術拖到合法從者，依卡面指定敵方或友方。無目標秘術拖到中央「秘術施放區」施放。拖到回收區優先回魔；放在非法位置、拖回手牌、右鍵或 Esc 都取消且不扣費。施放仍走 §5.1 的宣告付費與守方反制。

試玩方式與實際畫面見 [秘術拖放驗收](docs/arcana_drag_review.md)。
...
3. **Demo 打磨與可玩性深化（不含連線）** — 核心規則已可完整對局，但目前對局深度、手感與資源機制仍落後商用 CCG（爐石／闇影詩章 2）。公開 Demo 前優先完成：
   - **牌組體系化（固定套牌）**：從 132 卡池抽取配置，實作「快攻灼燒」與「死靈召喚」兩副固定牌組，取代開局全隨機發牌，讓玩家能打出明確流派 Combo。
   - **資源循環與過牌補給**：加入入場抽牌、亡語過牌或二選一濾牌，防止中後期進入「每回合抽一打一」的無選擇垃圾時間。
   - **後手平衡補償**：實作後手多抽 1 張起手牌，或給予「0 費暫時魔力幣（限一次性使用）」，平抑先手滾雪球優勢。
   - **卡槽防死鎖與空間博弈**：提供主動犧牲／回收卡槽或同路換位，防止低攻肉盾互卡前排造成戰局停滯；UI 高亮阻擋路線與守護範圍。
   - **戰鬥打擊感 (Game Feel)**：擺脫純數字扣減感；加入卡片衝刺位移、受擊卡牌晃動、Hit Stop 命中短暫停頓、重擊音效與鏡頭微震。
   - **超模翻盤點與 3D 可讀性**：設計條件降費（Mana Cheat）與 AOE 逆轉特異卡；優化遠端後排與血量 Label3D 遮擋問題。
   - **Demo 發佈整備**：音量控制（Audio Bus）、首局新手引導、UI 行動狀態提示、Windows 匯出 preset 驗收與 20 局單人實機試玩。完整盤點與驗收條件見 [docs/demo_readiness.md](docs/demo_readiness.md)，卡牌數值平衡修訂、待生成衍生卡與 HD-2D 像素卡圖候選清單見 [docs/todo_card_generation.md](docs/todo_card_generation.md)。
