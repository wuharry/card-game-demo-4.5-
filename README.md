...
- [x] 法術卡型卡面（§7 前置）：秘術/瞬咒/靈裝/伏印各 1 張**樣本卡**入池（`data/cards/`，效果文字標「結算待實作」）；非從者卡面＝圖示卡圖＋藏攻血＋卡型印章（`card.gd`），入槽仍被擋（既有 §7 閘門）。像素圖示包歸位 `assets/ui/icons/`（Shikashi v1/v2 免署名；Antahonist **CC-BY 4.0 發佈時需掛名 "Icons by Andrey Kalyuzhnyy"**）
...
1. **卡牌類型實作（結算層）** — 資料層與卡面已完成（`CardData.card_type` + 樣本卡 ×4 入池，見已完成清單）；剩各型**結算行為**：秘術出牌即結算、伏印蓋放與觸發、瞬咒反制窗口（[§5.1](#51-攻擊反制流程每次宣告-6-步)）、靈裝附著宿主；潛行/沉默關鍵字一併（它們只對法術有意義）。
2. **連線對戰（host-client，見 ADR-001；下一步從 2b 開始）** — 規則側（雙方帳、回合歸屬、牌堆權威）已由熱座完成，剩網路層，拆五小步、每步可獨立驗收：
   - ~~**2a. 大廳與連線骨架**~~ ✅ 完成（見已完成清單；`src/net/net_lobby.gd` + `net_match.gd` + 主選單大廳欄）
   - **2b. 回合與視角** — 熱座的「換邊重建手牌」改成「各自只看自己的手牌」；HUD 的我方/對方以**本機視角**顯示；非自己回合鎖操作（規則已有 `active_side` 檢查，補「本機是哪一側」的對照）。
   - **2c. 行動 RPC 管線** — client 送「意圖」（出牌到哪格／誰用什麼技能打誰）→ host 用現成 `*_block_reason` 驗證＋結算 → 廣播「結果」雙方播演出；抽牌只在 host 抽、結果（卡 id）廣播；結束回合同管線。
   - **2d. 對手手牌與牌堆同步** — 對手手牌只同步**張數**（顯示卡背扇形），牌堆剩量雙向同步。
   - **2e. 斷線與收尾** — 任一方斷線＝終局畫面（「對方已離線」）＋回主選單；大廳可取消等待。實測：本機開兩個視窗（`godot --path . & godot --path .`）對打一局。
