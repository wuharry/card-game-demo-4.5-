...
const HP_BADGE_TEXTURE: Texture2D = preload(
	"res://assets/ui/textures/battlefield/hero_health_badge.svg")
...
func _build_hp_label() -> void:
	# 紅盾只承擔可讀性，不包住角色；跟角色共用世界尺寸，避免遮住牌桌。
	var badge_pos := Vector3(0.68, CHAR_HEIGHT * 0.72, 0.02)
	_hp_badge = Sprite3D.new()
	_hp_badge.texture = HP_BADGE_TEXTURE
	_hp_badge.texture_filter = BaseMaterial3D.TEXTURE_FILTER_LINEAR_WITH_MIPMAPS
	_hp_badge.pixel_size = 0.0045
	_hp_badge.billboard = BaseMaterial3D.BILLBOARD_ENABLED
	_hp_badge.fixed_size = true
	_hp_badge.scale = Vector3.ONE * 0.12
	_hp_badge.no_depth_test = true
	_hp_badge.render_priority = 1
	_hp_badge.cast_shadow = GeometryInstance3D.SHADOW_CASTING_SETTING_OFF
	_hp_badge.position = badge_pos
	add_child(_hp_badge)

	_hp_label = Label3D.new()
	_hp_label.billboard = BaseMaterial3D.BILLBOARD_ENABLED
	_hp_label.font_size = 58
	_hp_label.outline_size = 12
	_hp_label.fixed_size = true
	_hp_label.scale = Vector3.ONE * 0.12
	_hp_label.no_depth_test = true   # 本體血量是勝負條件,永遠要讀得到
	_hp_label.render_priority = 2
	_hp_label.position = badge_pos
	add_child(_hp_label)
	_refresh_hp()
...
func _refresh_hp() -> void:
	...
	_hp_label.modulate = Color("ffd18a") if hp <= 5 else Color("f2ead8")
