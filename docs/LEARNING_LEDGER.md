# 學習債清單(Learning Ledger)

> Harness v2 對照：下文 CLAUDE.md §1a／§3／§6 分別對應現行入口的模式分派／Teaching／Learning；signal 中繼鏈移至 `.harness/project.md`。本帳本仍是 `godot.cardgame.*` 的權威，原有關卡、證據及退關紀錄完整保留。

> **這是什麼**:逃生艙 / AI 代工做出來、但 Harvey(Godot 新手)未必理解的觀念總帳。
> 建帳日 2026-07-06;主帳範圍 = commit `cbb80f4`…`f11931d`(Harvey 指定的逃生艙區間)。
>
> **給 agent 的使用規則**:
> 1. **出題時機**:只在 Harvey 主動說「**還債** / **考我** / **考我 §N** / **我要學會 X** / 我要複習 / 學以前的東西」時啟動,平常別轟炸(同 CLAUDE.md §6)。
>    **但「查等級」不受此限**——每次複雜任務分派前都要查(CLAUDE.md §1a),只是查完不一定出題。
> 2. **怎麼挑**:優先挑等級最低、最久沒驗的;超級新手 → 優先「第一梯隊·地基五柱」。
>    **一次 1 個概念**,該概念最多 2–3 題(不是一次考 3 個不同概念)。
> 3. **考完就記帳**:更新等級欄(附日期+判定依據)。答不出 → 照 CLAUDE.md §3 換方式教,教完維持原級、下次再驗,**不要教完就直接升級**。
> 4. **之後每次逃生艙任務收尾**:把新產生的觀念補進本表(預設「初階/未驗」)。

## 等級定義(Harvey 自訂)

| 等級 | 意義 |
|---|---|
| **初階** | 需要 agent 主動指導、主動提醒、或稍微修正才能運用 |
| **中階** | agent 問他能很好地回答,有不錯的熟悉度;**且能自己動手重建那段**(光答得好還不算) |
| **高階** | 能處理延伸題;做大任務完全不需幫忙;以前會卡的地方現在不卡 |

**升級判準**(2026-07-28 對齊 CLAUDE.md §6 鐵則:只有「**示範得出來**」才升等,光聽懂、光點頭不算):

- **初階 → 中階**:能答出「是什麼 + 為什麼這樣設計」**且**自己動手重建那段(§6 步驟 1 + 2)。
  單靠口頭答對**不升**——那只是四步的第一步。
- **中階 → 高階**:通過**新情境**的遷移題(§6 步驟 3),或在大任務裡自己把觀念用出來。

> 舊版判準寫「初階→中階 = 被問能答出『是什麼 + 為什麼』」,與 CLAUDE.md §6 鐵則衝突
> (§6 明文那只是第一步「預測」)。依 §6 為準修正——能複述是 fluency,能重建才是實力。

---

## 還債關卡表(2026-07-20 開帳;**這是進度的單一真相**,規則見 CLAUDE.md §6)

> 一關 = 一個核心觀念,底下的 § 是它在本帳的**現場**(同一門手藝的不同案發地)。
> **一關不等於一天**:過不了就同一關換講法再來,通常 1–3 天。第一~三梯隊按序必修,第四梯隊選修可跳。
> 狀態:`未開始` / `進行中` / **`已過`**(=中階) / **`精通`**(=高階) / `退關`(曾過但後來破功)

### 第一梯隊|地基五柱(每個後續任務都會再撞到)

| 關 | 核心觀念 | 現場(§) | 狀態 | 最近判定 |
|---|---|---|---|---|
| 1 | 模板 vs 實例:Resource 全場共享一份、Node 每個實例一份 | §7、§18、§27、§34 | 未開始 | — |
| 2 | 帳與視圖分離:帳是真相、視圖只是投影(含對帳點) | §21、§25、§27、§28、§41 | 未開始 | — |
| 3 | 基準值要快照、即時值才即時讀 | §5、§16、§18、§29 | 未開始 | — |
| 4 | choke point:找「所有路都會經過的窄口」,一次接線 | §20、§26、§34、§35、§36、§37、§39 | 未開始 | — |
| 5 | signal 的執行期本質:廣播 vs 點名;簽名錯要發射才炸 | CLAUDE.md 中繼鏈、§36 | 未開始 | — |

### 第二梯隊|debug 生存包(研究說外包 AI 最傷 debug 力,這包直接補)

| 關 | 核心觀念 | 現場(§) | 狀態 | 最近判定 |
|---|---|---|---|---|
| 6 | class_name 全域快取三連環+「pull 後先 `--headless --editor --quit`」 | §19、§22、§30、§37、§38 | 未開始 | — |
| 7 | 「執行期才炸」的錯誤家族:哪些錯 parse 期抓得到、哪些要跑到那行 | §19、§21、§35、§36 | 未開始 | — |
| 8 | 懸空參考家族:`is_instance_valid` 是保險不是門 | §18、§21、§26、§29、§37 | 未開始 | — |
| 9 | headless 驗證工具箱+測試要有牙齒(假綠燈) | §12、§19、§30、§32(UPnP)、§34 | 未開始 | — |
| 10 | 錯誤定位術:除錯器截停的長相、IDE 紅字≠引擎判決、對照實驗砍嫌疑犯 | §21、§23、§40 | 未開始 | — |

### 第三梯隊|下個任務的前置知識

| 關 | 核心觀念 | 現場(§) | 狀態 | 最近判定 |
|---|---|---|---|---|
| 11 | `await` 協程做流程暫停點(含 await 後世界已變) | §11、§18、§27、§28 | 未開始 | — |
| 12 | 連線同步三件套+「零同步紅利」為什麼是複利 | §22、§25、§28、§30、§34、§36 | 未開始 | — |
| 13 | 規則的單一出口+理由字串;邊界順序決定規則正確性 | §18、§19、§20 | 未開始 | — |
| 14 | NAT / 對外 IP / UPnP 打洞(對照 [ADR-001](adr-001-network-multiplayer.md)) | §32(UPnP) | 未開始 | — |

### 第四梯隊|視覺演出(前三梯隊穩了再挑有興趣的,一次一個,順序隨意)

| 關 | 核心觀念 | 現場(§) | 狀態 | 最近判定 |
|---|---|---|---|---|
| 15 | shader:GPU 每像素跑、`TIME` 驅動、UV→極座標 | §1、§17、§39、§40 | 未開始 | — |
| 16 | 粒子系統三件套與一次性爆點 | §15、§26 | 未開始 | — |
| 17 | 相機構圖是「角度預算」+ DOF 與透明物件的深度陷阱 | §29、§32(景深) | 未開始 | — |
| 18 | UI 尺寸誰決定:錨點=比例、MINSIZE 快照、節點選型 | §11、§16、§36 | 未開始 | — |
| 19 | 素材管線:匯入邊界、AtlasTexture、引用熱度分級 | §8、§23、§33、§37 | 未開始 | — |
| 20 | 程序生成:噪聲 vs 隨機、GridMap、散佈取樣、用量的不用猜的 | §2、§3、§10、§12、§19、§39 | 未開始 | — |

**速贏區(差半步就升級,想換口味時可插隊)**:§9 出血(只缺「凸出被框蓋住」那半)、
§23 圖塞窗三策略、§5 牙籤偏移(中階→高階)、§12 水面最低點(中階→高階開藥方)。

> 📌 註:本帳有**兩個 §32**(景深 / 異地連線 UPnP),沿用原編號未動以免打斷既有交叉引用,
> 引用時一律加括號區分。

---

## 建議學習順序(2026-07-17 排定;上面關卡表的排序依據)

> 排序依據:**觀念在本專案的出現次數 × 之後任務還會不會再撞到**,不是 commit 時間順序
> (commit 順序是功能需要的順序,不是知識依賴的順序)。同梯隊內先挑等級最低的。

**第一梯隊|地基五柱(每個之後的任務都會再撞到,先學投報率最高)**
1. 模板 vs 實例 / Node vs Resource 生命週期 —— §7、§18、§26、§34(全帳出現最多次)
2. 帳與視圖分離(帳=真相,視圖=投影)—— §21、§25、§27、§28(本專案架構主幹)
3. 基準值要快照、即時值才即時讀 —— §5 源頭;§16、§18、§29 是同族現場
4. choke point 單一入口 —— §20 傷害管線、§26 特效、§34 bury、§36 _pay、§37 音效(同一門手藝五個現場)
5. signal 的執行期本質(廣播 vs 點名;簽名錯誤發射才炸)—— CLAUDE.md 中繼鏈、§36

**第二梯隊|debug 生存包(研究說外包 AI 最傷的就是 debug 力,這包直接補)**
6. class_name 全域快取三連環+「pull 後先 `--headless --editor --quit`」 —— §19、§30、§38
7. 「執行期才炸」的錯誤家族:帶型別陣列(§19)、屬性搬家(§21)、信號簽名(§36)——核心問題:哪些錯 parse 期抓得到、哪些要跑到那行才爆?
8. 懸空參考家族+is_instance_valid 是保險不是門 —— §18、§21、§29
9. headless 驗證工具箱+測試要有牙齒(假綠燈)—— §12、§19、§32、§34
10. 除錯器截停的長相(失焦卡住 ≠ crash)—— §21(小而救命)

**第三梯隊|下個任務的前置知識(堆疊/反制窗+連線驗證要用)**
11. await 協程做流程暫停點 —— §27(LIFO 堆疊動工前必懂)
12. 連線同步三件套:開局一致+操作重放、跨機身分證、出牌端/重放端分流 —— §28
13. NAT/UPnP/區網 vs 對外 IP —— §32(異地連線)
14. 規則單一出口+理由字串 —— §18

**第四梯隊|視覺演出(前面穩了再挑有興趣的,一次一個)**
shader 家族(§1、§17、§35、§38)、DOF 與透明深度(§32 景深)、粒子(§15、§26)、
相機構圖(§29)、UI 尺寸誰決定(§11、§16、§36)、素材管線(§8、§23、§33、§37)

**速贏區(差半步就升級,考這幾個最划算)**:§9 出血(只缺「凸出被框蓋住」那半)、
§23 圖塞窗三策略(已能自己取捨)、§5 牙籤偏移(中階→高階延伸題)、§12 水面最低點(中階→高階開藥方)。

---

## 主帳(cbb80f4…f11931d)

### 1. 溪流水面 shader(`cbb80f4`;[assets/water/stream_water.gdshader](../assets/water/stream_water.gdshader))

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| shader 在哪跑:fragment 每幀·每像素·在 GPU;跟 `_process`(CPU、每幀一次)的量級差 | 初階/未驗 | 「水面波紋一秒被算幾次?搬到 `_process` 用 CPU 算會發生什麼?」 |
| 法線貼圖騙光:波紋是「動法線騙光照」,網格根本沒動 | 初階/未驗 | 「把相機貼平水面側看,水面是平的還是起伏的?那為什麼正看有波?」 |
| `TIME` 驅動 UV 捲動(動畫的來源) | 初階/未驗 | 「波紋為什麼會流?TIME 乘 2 畫面變什麼樣?」 |
| SSR 螢幕空間反射:只能反射「當前畫面裡有的東西」 | 初階/未驗 | 「為什麼樹一被鏡頭切出畫面,水裡倒影就消失?」 |

### 2. 程序化地板(`785c975`、`598cd03`;[src/environment/ground_generator.gd](../src/environment/ground_generator.gd))

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 噪聲 vs 純隨機:FastNoiseLite「相鄰值相近」所以成簇;`randi` 是雪花點 | 初階/未驗 | 「想要泥巴斑塊更大顆,調 frequency 還是 threshold?兩者各控制什麼?」 |
| 閾值切分:連續噪聲值 → 離散磚種(草 / 泥 / 過渡磚) | 初階/未驗 | 「過渡磚是怎麼被選中的?畫出噪聲值軸上三個區間」 |
| GridMap + MeshLibrary 的分工(棋盤格 vs 磚的字典) | 初階/未驗 | 「新增一種磚要動哪邊?鋪磚邏輯要改嗎?」 |
| `598cd03` 的 bug 根因:磚隨機朝向 → 部分磚光照/UV 方向錯 → 深色斑塊 | 初階/未驗 | 「為什麼『轉一下磚』會讓它變深色?跟法線的關係?」 |

### 3. 森林散佈([src/environment/forest_scatter.gd](../src/environment/forest_scatter.gd))

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 成簇散佈(先選簇心、再繞著撒點)與內圈淨空(排除區判定) | 初階/未驗 | 「為什麼牌桌正中央永遠不會長樹?淨空是在哪一步被保證的?」 |
| alpha scissor(鏤空)vs alpha blend:樹葉用 scissor 是為了躲透明排序問題 | 初階/未驗 | 「樹葉改用 alpha blend 會出什麼視覺 bug?為什麼 scissor 沒這問題?」 |
| 生成的節點不設 owner → 不會存進 .tscn(場景檔保持薄,內容由 code 重建) | 初階/未驗 | 「編輯器裡看得到幾百棵樹,.tscn 檔裡卻沒有它們——為什麼?這樣的好處?」 |

### 4. 卡牌互動 code review 清理(`80b6ed8`)

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 封裝:`play_card()` 收進 PlayerHand,外部不直接動 `cards` 陣列 | 初階/未驗 | 「如果 CardManager 直接 `cards.erase(card)`,少做了什麼事?會出什麼 bug?」 |
| `@export` 節點注入 vs `$` 字串路徑(`node_paths` 的意義;場景樹搬家誰會壞) | 初階/未驗 | 「把 PlayerHand 在場景樹搬到別層,@export 注入和 `$../PlayerHand` 誰活誰死?」 |
| 靜態型別標註的好處(自動補全、錯誤提早在編輯期爆) | 初階/未驗 | 「`var card: Card` 比 `var card` 多買到哪兩件事?」 |

### 5. Label3D 深度與數值顯示(`493de31`、`57499e0`)——已教學

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| local 偏移跟著節點旋轉走(牙籤模型):z 偏移只有面向相機時才是「深度」 | **中階**(2026-07-06 理解題通過:垂直俯視→貼合,含平躺卡前提推理正確) | 升高階:給新情境(如怪物頭上血條在斜視角穿模)讓他自己開藥方 |
| z-fighting 根因與三種解法的取捨:縮短偏移 / render_priority / no_depth_test | 初階/未驗(修法是 AI 給的) | 「no_depth_test 什麼時候該用?在手牌扇形重疊下的代價是什麼?」 |

### 6. WorldEnvironment 後製棧([scenes/arena_forest.tscn](../scenes/arena_forest.tscn) 內)

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| glow 與 HDR 門檻:亮度超過 `glow_hdr_threshold`(0.95)才泛光;emission 是主要推手 | 初階(2026-07-06 教過,未驗) | 「為什麼 emission 3.0 會泛光、0.5 不會?門檻設定在哪個檔的哪個節點?」 |
| ACES tonemap / SSAO / 霧 各自解決什麼問題 | 初階/未驗 | 「關掉 SSAO 畫面會少什麼?tonemap 是在管什麼的?」 |
| 設定檔「沒寫 = 預設值」:`rendering_method` 缺席 = forward_plus;features 標籤 ≠ 開關 | 初階(2026-07-06 教過,未驗) | 「project.godot 找不到某設定鍵,代表什麼?去哪確認地面真相?」 |

### 7. 本 session(2026-07-06)新教觀念,一併記帳

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 資料/視覺分離:會變的 = 資料 + 文字節點;不變的外觀 = 圖 | 初階(火球術題答半對:知道差在資料層,但誤以為「資料 = 改 code」) | 「新增一張卡要動哪些檔、不動哪些?」→ 親手做過 .tres 後再驗 |
| Resource 預設共享 vs Local to Scene:Node 每實例一份、Resource 全場一份 | 初階(教過,未驗) | 「不開 Local to Scene,10 個卡槽為什麼一起亮?CardData 的共享為什麼反而是優點?」 |
| 扇形間距幾何:`sin(gap)×radius` vs 卡寬 決定重疊 | 初階/未驗 | 「手牌變 7 張會發生什麼?哪個上限先卡住?」 |

---

## 續帳(f11931d…HEAD,本機 session;2026-07-06 依 session 全記錄補齊)

> 原「待分類」五條全部展開移入。等級照規則預設「初階/未驗」;
> 有實際問答依據的兩處已標註(§12 河道根因 = 中階、§9 出血 / §11 錨點 = 出過題未答)。

### 8. CardData 資料層實作(`dda702a`;[src/card/card_data.gd](../src/card/card_data.gd)、[data/cards/](../data/cards/))

> 觀念層(資料/視覺分離)在主帳 §7,這裡是實作層。

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| .tres 檔解剖:ext_resource(引用外部檔)vs sub_resource(內嵌小資源) | 初階/未驗 | 「打開 knight.tres:哪行是引用、哪行是內嵌?刪掉被引用的 png 會發生什麼?」 |
| AtlasTexture:在大圖上「框一格」當獨立貼圖(卡圖佔位 = 動畫表第 0 幀) | 初階/未驗 | 「region = Rect2(0,0,100,100) 在說什麼?想改用第 3 幀改哪個數字?」 |
| DirAccess 掃資料夾 + 懶載入(卡池第一次要用才載);`.remap` 後綴剝除 | 初階/未驗 | 「卡池是開遊戲瞬間載入,還是第一次發牌才載?懶載入買到什麼?」 |

### 9. 卡圖挖空窗與召喚立牌(`016cb9a`、`42e4dfc`;[src/card/card.gd](../src/card/card.gd))

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 透明繪製順序:卡圖擺卡框「後面」(z=-0.01)從挖空窗露出 → 怎麼縮放都壓不到框的美術 | 初階/未驗 | 「為什麼放後面反而安全?改成 +0.01 疊前面會出什麼事?」 |
| 出血(bleed):遮罩決定形狀、內容做大藏在遮罩後——卡圖出血 6% 和水面板加寬是同一條原則 | 初階(2026-07-05 出題未答;**2026-07-11 進步**:圖示 cover 題自己推出「畫大會凸出」,但「凸出被框蓋住所以沒事」仍由 agent 補完,補齊這半即可升中階) | 「bank_jitter 調到 2.0,水面板要多寬才不露餡?公式?」 |
| 可見範圍掃描:alpha 掃出角色實佔行列,大小/腳位用「量出來的」——100×100 格子裡角色只有 ~30px,信帳面就會又小又飄 | 初階/未驗 | 「按整格算,角色為什麼只剩帳面 1/3 還懸空?掃描一次解決哪兩件事?」 |
| Sprite3D 立牌三件套:hframes 切格(幀數=寬÷高)、tween_callback+set_loops 播待機、BILLBOARD_FIXED_Y 直立面向鏡頭 | 初階/未驗 | 「幀數 6 是誰算出來的?為什麼這批素材能用寬除以高?」 |
| 旋轉疊加(§5 牙籤原則的旋轉版):place_card 轉 local (0,0,0) 就是躺平——父節點 PlayerHand 已 -90°X;再補 -90 卡會立起來 | 初階/未驗 | 「卡在槽裡 local rotation=0,世界裡為什麼是躺的?『躺』是誰給的?」 |

### 10. 戰場家族與輪替(`3d9d5da`;[src/environment/arena_base.gd](../src/environment/arena_base.gd)、[arena_pool.gd](../src/environment/arena_pool.gd))

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 場景繼承:共同流程在基底(環境/地板/散佈工具),子類只覆寫 `_build()` | 初階/未驗 | 「新增『沙漠』牌桌要寫哪些函式、哪些直接繼承來用?」 |
| static 類別變數跨場景傳值:static 活在類別上,change_scene 清不掉(ArenaPool 不用 autoload 的原因) | 初階/未驗 | 「換場景後 main.tscn 為什麼還讀得到抽籤結果?什麼時候該升級成 autoload?」 |
| free() vs queue_free():queue_free 等幀尾才刪;換 WorldEnvironment 必須立刻 free,兩環境並存一幀會打架 | 初階/未驗 | 「哪種情境不能等幀尾?main_scene.gd 那行為什麼用 free()?」 |

### 11. 主選單(`d36e167`;[src/main_menu/main_menu.gd](../src/main_menu/main_menu.gd))

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 錨點=比例、偏移=像素修正;`set_anchors_preset` 只動錨點且「保留當下矩形」→ UI 鎖死在舊視窗大小(置中 bug 根因);帶 `and_offsets` 的才等於 Layout 選單 | 初階(2026-07-05 教過+出題未答) | 「編輯器裡擺好的 Control 呼叫 set_anchors_preset 會不會歪?跟 code 生成的 0×0 新節點差在哪?」 |
| CanvasLayer:2D 疊層永遠畫在 3D 之上(選單、黑幕都掛它下面) | 初階/未驗 | 「UI 不掛 CanvasLayer、直接掛在 Node3D 下會發生什麼?」 |
| 主題覆寫做文字選單:normal 給 StyleBoxEmpty、hover/focus 共用「只開下邊框」的 StyleBoxFlat → 滑鼠/鍵盤回饋一致 | 初階/未驗 | 「為什麼 focus 也要接樣式?只接 hover 冷落了誰?」 |
| 轉場模式:鎖按鈕 → tween 黑幕 → `await tw.finished` → change_scene_to_file,失敗要還原 | 初階/未驗 | 「await 那行在等什麼信號?不鎖按鈕連點兩下會怎樣?」 |

### 12. 森林地形整修(`04f8bbf`;[ground_generator.gd](../src/environment/ground_generator.gd)、[forest_scatter.gd](../src/environment/forest_scatter.gd))

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 水面必須是「局部最低點」:換材質治標、換高度治本 | **中階**(2026-07-05 自己推出根因與修法方向) | 升高階:新情境開藥方——「岩漿池 / 流沙坑要怎麼做才不浮?」 |
| 磚是紙片、貼 cell 正中央:面高度 = (y_level + 0.5) × cell_size.y = -0.5;「地表在 0」是幻覺,樹和水都因此浮過 | 初階(兩個高度數字自己查到;薄片幾何是 AI 量的) | 「y_level -3、cell_size.y 0.2,地表在哪?想要 0.1 深的河改哪兩個數字?」 |
| cell_size.y 當垂直解析度:GridMap 只能整格跳,格距改細 = 垂直步距變細(1 → 0.2) | 初階/未驗 | 「為什麼不能『把河床磚往下移 0.2』,非得動 cell_size?」 |
| 薄片模型處置:兩片十字交叉才不會側面消失;用 AABB 反推貼地 / 露頂高度(土丘埋地只露丘頂) | 初階/未驗 | 「_make_prop 怎麼判斷要不要交叉?土丘的 Y 是怎麼算出來的?」 |
| 面積均勻取樣:環帶內半徑要用 sqrt(rand) 插值,否則點擠內圈 | 初階/未驗 | 「為什麼要開根號?不開的話點擠哪邊、為什麼?」 |
| headless Godot 當診斷工具:`--headless -s` 跑一次性腳本印 AABB——「用量的代替用猜的」 | 初階/未驗 | 「想知道某個 mesh 的實際尺寸和原點,除了拖進場景目測,還有什麼辦法?」 |

### 13. 工作流的坑(跨任務,反覆咬人)

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 編輯器舊緩衝蓋檔:外部改過 .gd/.tscn 後回 Godot,提示一律選 **Reload**,否則一存檔就把改動蓋回舊版 | 初階(本 session 踩過多次) | 「什麼情況會跳這個提示?選錯會發生什麼、怎麼發現?」 |
| `echo >> .gitignore` 的黏行陷阱:原檔末行沒有換行符時,append 會黏成一行廢規則(assets/_Raw_Imports/build/)——不但新規則沒生效,舊規則也被弄壞;154MB 匯出 zip 因此差點進版控(amend 救回) | 初階(2026-07-12 AI 自踩自修) | 「為什麼 grep 得到 build/ 卻沒忽略成功?怎麼一眼檢查檔案有沒有結尾換行?git 已 commit 的大檔怎麼救?」 |

### 14. 技能資料層設計(2026-07-09;[skill_data.gd](../src/card/skill_data.gd)、[skills_design.md](skills_design.md))

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| Resource 巢狀:CardData 裡掛 SkillData 子資源(.tres 會存成 sub_resource),資料樹跟場景樹是兩回事 | 初階/未驗 | 「active_skill 存在 .tres 的哪一段?跟 standee 的 ext_resource 差在哪?」 |
| enum 當「系統合約」:Kind/Modifier/Effect 枚舉刻意壓小 = 戰鬥系統只要實作這幾種;加新招是加枚舉值+資料,不是加 if | 初階/未驗 | 「想加一招『沉默目標』,要動 skill_data.gd 的哪裡?戰鬥系統為什麼不用改架構?」 |
| 兄弟資源路徑推導:由 standee 路徑字串切「最後一個底線」推 Attack02 等動畫表;ResourceLoader.exists 防呆 | 初階/未驗 | 「為什麼用 rfind 而不是 replace(\"_Idle\", ...)?蝙蝠(無 Idle)哪裡會踩雷?」 |
| 動畫=能力的設計錨:先盤點素材(誰有 Attack02/Block/Summon)再配技能,有格擋動畫才有鐵壁被動 | 初階/未驗 | 「為什麼骷髏弓手沒有主動技?骷髏家族的不滅是從哪個素材事實推出來的?」 |

### 15. 雪地/洞窟場景優化(2026-07-09;[arena_frostlands.gd](../src/environment/arena_frostlands.gd)、[arena_caverns.gd](../src/environment/arena_caverns.gd)、[arena_base.gd](../src/environment/arena_base.gd))

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| GPUParticles3D 三件套:ProcessMaterial(發射盒/速度/亂流)、draw_pass(QuadMesh+billboard)、`preprocess`(開場即滿不用等) | 初階/未驗 | 「為什麼開場第一秒就有雪?visibility_aabb 設太小會出什麼詭異 bug?」 |
| 粒子貼圖可以程式生:GradientTexture2D 放射漸層 = 幾像素的小白點,不用進美術資源 | 初階/未驗 | 「雪花貼圖是哪來的?想把雪花變成六角形要換哪個環節?」 |
| 剪影策略:倒吊鐘乳石(rotation.x 180°)+ 黑暗與霧 = 大腦自己補完「洞頂」,不用真的蓋天花板 | 初階/未驗 | 「垂刺為什麼不用登記 _placed?吊多低會穿幫?」 |
| 跨包混材質:模型的 UV 對誰的 tilesheet 就接誰的表——冰晶到洞窟還是接 frostland 總表,不是接洞窟的 | 初階/未驗 | 「把冰晶接 Cavern_Tilesheet 會發生什麼?為什麼?」 |
| 配色公式「大面冷色+少量暖點」(雪夜火把)/「大面暖色+少量冷點」(洞窟藍水晶):對比色是畫面的收束點 | 初階/未驗 | 「洞窟裡的藍水晶在配色上的職責是什麼?拿掉會怎樣?」 |

### 16. 指令選單與技能演出(2026-07-09;[battle_ui.gd](../src/battle_ui/battle_ui.gd)、[card_manager.gd](../src/card_manager/card_manager.gd))

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 互動狀態機:同一顆左鍵在 IDLE/DRAGGING/MENU_OPEN/TARGETING 代表不同動作,集中在一個 enum 分流,不靠散落的 bool 旗標 | 初階/未驗 | 「為什麼不用 is_dragging + is_menu_open 兩個 bool?狀態機多買到什麼?」 |
| 「碰撞關掉=射線打不到」的反面:上桌卡片要可點擊,鎖定就不能再用停用碰撞實作,改用旗標分流 | 初階/未驗 | 「舊的 lock_interaction 為什麼會讓指令選單永遠開不起來?」 |
| UI 問、Manager 決定:BattleUI 只發信號(attack_chosen…),狀態轉移全在 CardManager——UI 換皮不動邏輯 | 初階/未驗 | 「把選單改成寶可夢 2×2 格,要動哪支檔?CardManager 要不要改?」 |
| 演出與規則分離:發動只播動畫+emit action_performed,結算留給未來戰鬥系統訂閱——同一條信號兩個階段用 | 初階/未驗 | 「戰鬥系統動工時要在哪裡接?_execute_action 需要改嗎?」 |
| .tres 巢狀資源實戰:sub_resource + script = SkillData 實例;headless 批次載入當資料驗證(bad=0 才算接完) | 初階/未驗 | 「Skill_main 這個 sub_resource 是怎麼變成 SkillData 的?驗證腳本驗了哪兩件事?」 |
| git 暫存區與工作區是兩本帳:檔案刪了但 index 還留著 A,照樣會被寫進歷史(字型 zip 事件) | 初階(2026-07-05 教過) | 「status 顯示 `AD` 是什麼狀態?怎麼把它從暫存區退掉?」 |
| 刪資源的順序:先把所有引用(.tres/.gd)退掉 → 再走編輯器 FileSystem 刪,不用 rm | 初階(2026-07-06 教過) | 「先刪圖再改引用會出什麼事?為什麼要走編輯器刪?」 |
| 量尺寸的時機:PRESET_MODE_MINSIZE 是「當下最小尺寸」的快照——面板還空著就定位=量到 0,內容進來後往預設方向(右下)長出螢幕外只剩標題列;修法=內容就位後重新定位+grow_vertical 往上長。與 original_scale 快照同族:基準要在資料就位後才拍 | 初階/未驗 | 「open() 裡的 reset_size+重新 set_anchors_and_offsets_preset 拿掉,選單會變怎樣?為什麼?」 |
| 2D↔3D 座標往返:project_ray_*(螢幕→3D 射線)與 unproject_position(3D→螢幕像素)互為反運算——導引箭頭=每幀把施放者的 3D 位置投回螢幕,用 Line2D 畫二次貝茲弧線;鎖定目標時尖端吸附目標(吸附=把類比輸入折算成明確意圖的回饋) | 初階/未驗 | 「箭頭起點為什麼要每幀重算而不是進 TARGETING 時算一次?哪種情況下只算一次會出錯?」 |

### 17. 卡槽高亮著色器(2026-07-10;[slot_tile.gdshader](../src/card_slot/slot_tile.gdshader)、[card_slot.gd](../src/card_slot/card_slot.gd))

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| SDF(有號距離場)畫程序化 UI 形狀:sd_rounded_box 回傳「到邊緣的距離」,一個距離值就能同時做外框線(abs(d)≈0)、第二道內線(d+0.045)、內緣漸層(d<0 的 smoothstep)——形狀是「算」出來的,不是貼圖 | 初階/未驗 | 「要再加第三道更靠內的細線,改哪個數字?為什麼 abs() 能讓一條線出現在邊的兩側?」 |
| 非等比縮放會把圓角拉成橢圓:先把 UV 乘上「寬/深」壓回等比空間再算 SDF(aspect uniform 由 GDScript 從節點實際 scale 算出傳入) | 初階/未驗 | 「拿掉 aspect 校正,卡槽四個角會變成什麼樣?為什麼?」 |
| shader_parameter/* 是 shader 解析後才存在的「動態屬性」:tween_property 綁它在 headless(dummy 渲染器)下會炸;tween_method + set_shader_parameter 不依賴算繪器狀態。搭配「從當下值起跑」避免動畫中途反向時跳變 | 初階/未驗 | 「為什麼 _current_glow() 不直接回傳 0 或 1?什麼操作序列會讓寫死起點的版本跳一下?」 |
| 執行期換材質:_ready 裡 new ShaderMaterial 蓋掉場景材質 → .tscn 零改動、每個實例一份材質(各槽發光獨立);headless 驗證的時序陷阱:SceneTree 腳本 _init 時 _ready 還沒跑,要 await process_frame | 初階/未驗 | 「20 個卡槽共用一份 ShaderMaterial 會出什麼視覺 bug?」 |

### 18. 魔力與生命值(2026-07-10;[battle_manager.gd](../src/battle_manager/battle_manager.gd)、[card.gd](../src/card/card.gd))

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 模板 vs 實例:CardData 是 24 張卡共用的 Resource,當前血量絕不能寫回模板(寫了=全場同名卡一起掉血);會變動的執行期狀態(current_hp、行動旗標)放場上的 Card 實例身上 | 初階/未驗 | 「把 current_hp 存進 CardData 會出什麼 bug?兩張「士兵」同場時會發生什麼?」 |
| 「同時結算」的實作:雙向傷害交換要先把反擊值記下來再一起扣——若順序扣血,先歸零的一方就打不出反擊,規則就錯了(交戰前快照,和 original_scale 同族) | 初階/未驗 | 「_resolve_attack 裡的 counter 變數為什麼要在 take_damage 之前取?刪掉先取的動作會怎樣?」 |
| 規則的單一出口:合法性檢查回傳「被擋的理由字串」(""=可做),UI 拿去灰化+轉述、發動前再驗一次——按鈕永遠只是轉述,最後一道門在帳房 | 初階/未驗 | 「為什麼 _on_attack_chosen 還要再查一次 attack_block_reason?UI 已經灰化了不是嗎?」 |
| 死亡的收尾順序:先清卡槽(位子馬上能用)→ 廣播 unit_died(讓別人清參考)→ 再播死亡演出+queue_free;拿著 freed 物件的參考=懸空參考,摸下去就炸 | 初階/未驗 | 「CardManager._on_unit_died 不清 hovered_target 會在什麼操作下炸掉?」 |
| await + create_timer 做「演出對時」:結算延後 0.35 秒讓數字跟拳頭一起落地;await 之後世界可能已變(單位死了),恢復執行前要 is_instance_valid 再驗 | 初階/未驗 | 「on_action_performed 裡 await 後面那句 is_instance_valid(caster) 防的是什麼劇本?」 |
| 看不見的規則=沒有的規則:反擊有算但沒演,玩家就以為對面攻擊力比較高——回饋(飄浮數字/受擊動畫)要綁在「結算事件」上,不是綁在「宣告動作」上(被治療播受傷就是綁錯邊的症狀);飄浮數字用 no_depth_test + billboard 保證讀得到 | 初階/未驗 | 「受擊動畫為什麼從 _execute_action 搬到 _resolve_attack?搬之前治療隊友會發生什麼?」 |

### 19. 本體、打臉與勝負(2026-07-10;[hero.gd](../src/hero/hero.gd)、[battle_manager.gd](../src/battle_manager/battle_manager.gd))

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| class_name 的全域類別快取:新腳本的 class_name 要等編輯器掃描(或 `--headless --import`)才進 `.godot/` 快取——沒進快取前,headless 連「引用它的其他腳本」都解析失敗;IDE 的 "Could not find type" 同源 | 初階/未驗 | 「為什麼加了 hero.gd 之後,連 battle_manager.gd 都在 headless 下 parse error?怎麼修?」 |
| 兩種型別一個介面:Card 和 Hero 都有 animate_hover/take_damage,共用變數宣告成共同祖先 Node3D,結算用 `target is Hero` 分流——先想「誰的規則不同」(反擊有無)再決定分流點,不是到處 if | 初階/未驗 | 「為什麼 hovered_target 從 Card 改宣告成 Node3D?打臉為什麼在 on_action_performed 分流而不是 _resolve_attack 裡?」 |
| 位置對位不寫死索引:路線(lane)判定用「x 座標最近的對面前排格」,換戰場/改棋盤排法不用回來改;本體站位同理由卡槽群組實際位置推算(call_deferred 等群組生完) | 初階/未驗 | 「_lane_blocked 為什麼不用『第 i 欄對第 i 欄』?卡槽間距改了會不會壞?」 |
| 終局閘門:game_ended 一票否決(行動合法性第一條就擋)+ UiState.GAME_OVER 鎖 3D 輸入——「遊戲結束」要同時關掉規則層和互動層,只關一層會漏(結算 await 中途分出勝負的殘餘行動) | 初階/未驗 | 「勝負已分後,為什麼 face_block_reason 和 ui_state 兩邊都要擋?只擋 UI 會發生什麼?」 |
| 三元運算式 vs 帶型別陣列:`a if c else b` 產出的是無型別 Array,執行期指派給 Array[String] 直接炸(而且是「執行到才炸」,parse 過得了)——帶型別容器要用直述句逐一指派;headless 跑主場景 N 幀是抓這類「啟動期執行錯誤」的網 | 初階/未驗 | 「為什麼這個 bug 編輯器不會標紅字、F5 才爆?headless --quit-after 驗的是哪一類錯?」 |
| 站位要讓鏡頭與 UI 的地盤:我方棋盤「正後方」在畫面上就是手牌扇形的位置,3D 物件站那裡必被 UI 擋——擺東西前先想「這個世界座標投影到螢幕是哪一塊」;duplicate() 可整棵複製現成節點樹做鏡射視覺(敵方牌堆),不用重刻場景 | 初階/未驗 | 「敵方牌堆的 z 為什麼是 mid_z*2 - deck.z?這條公式在做什麼幾何操作?」 |
| 網格地形的「方齒」成因:1m 方格上跑高頻噪聲,相鄰格推擠量差超過一格就擠出單格凸齒(看起來像掉了一塊素材)——蜿蜒感靠「幅度」出、不靠高頻抖動(頻率 0.13→0.05,彎改跨 4~8 格);磚緣直線再用水線雜物(岸石/露頭石/水草)遮參差,轉場處的髒亂是藏縫的標準手法 | 初階/未驗 | 「同樣的 bank_jitter,為什麼頻率高會出方齒、頻率低就是河灣?水線石頭為什麼要兩成丟進水裡?」 |

### 20. 狀態效果、打法修飾與卡型預留(2026-07-10;[battle_manager.gd](../src/battle_manager/battle_manager.gd)、[card.gd](../src/card/card.gd))

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 傷害管線單一入口:所有扣血都走 _deal_damage(夜幕減半→鐵壁-1→扣血+演出),減免規則只寫一份;吸血拿的是管線回傳的「實際傷害」不是帳面值——資料流設計:先想清楚「誰需要這個計算的結果」 | 初階/未驗 | 「吸血遇到鐵壁目標,回多少血?為什麼減免不能寫在 take_damage 裡?」 |
| 邊界順序決定規則正確性:回合切換=結束 tick → 回合+1 → 開始 tick → 狀態遞減——「先 tick 再遞減」1 回合灼燒才咬得到結束+開始共 2 點;順序反了規則就默默少一半 | 初階/未驗 | 「把 decay_statuses 移到開始 tick 之前,灼燒 1 回合會咬幾點?」 |
| enum 第 0 項=預設值的零遷移技巧:CardType.MINION 排第 0,24 張舊 .tres 一張不用改就自動是從者——擴充資料 schema 時,讓「舊資料的隱含值」正好是新欄位的預設值 | 初階/未驗 | 「如果把 ARCANA 排在 enum 第 0 項會發生什麼?」 |
| 爆牌制的設計取捨:爐石/暗影詩章式「滿手抽牌即銷毀」vs MTG 式「回合末棄到上限」vs 不抽——爆牌懲罰明確+規則最簡,且與本作丟牌回魔(§1.1)形成「囤牌有代價、棄牌有價值」的閉環 | 初階/未驗 | 「為什麼『手滿就不抽』會鼓勵拖節奏?爆牌制怎麼反制囤牌?」 |
| 副目標不反擊的簡化:橫掃/貫穿的濺射目標只挨打不還手(反擊只屬於主目標的雙向交換)——多目標規則先訂「誰有資格反擊」,不然結算順序組合爆炸 | 初階/未驗 | 「橫掃打三個目標,如果三個都反擊,攻擊者會吃幾刀?規則書為什麼不這樣設計?」 |

### 21. 真牌堆與熱座化(2026-07-10;[deck.gd](../src/card/deck.gd)、[battle_manager.gd](../src/battle_manager/battle_manager.gd)、[player_hand.gd](../src/play_hand/player_hand.gd))

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 帳與視圖分離:手牌的「帳」(Array[CardData])在 BattleManager 的 SideState,PlayerHand 只是投影——換邊=stash 現況→換帳→rebuild 視圖;連線時把「對面的視圖」拿掉就是網路版,規則層一行不用改 | 初階/未驗 | 「熱座換邊的三步驟(stash/end_turn/rebuild)順序能不能對調?為什麼視圖不能當真相?」 |
| 連線前先熱座:把「雙人規則」(雙帳/回合歸屬/側別檢查)在零網路環境做完並驗證,之後 ENet 只負責「把對面的操作送過來」——規則錯誤和網路錯誤拆成兩個獨立的 debug 空間 | 初階/未驗 | 「為什麼不直接上 ENet?熱座版驗過的哪些東西到網路版不用重驗?」 |
| 公平的資源成長:魔力上限在「輪到自己回合開始」才 +1(雙方各自的 _begin_side_turn),不是全域回合數推——否則後手第一回合就有 2 魔;每側的狀態效果也只在持有者自己的回合階段 tick | 初階/未驗 | 「如果魔力成長寫在全域 end_turn 裡,先手/後手的魔力曲線會差多少?」 |
| 內部類 class SideState:同形狀的兩份帳用一個類裝,dict 選 side——比 mana_p/mana_e 平行變數好擴充(加欄位一處改),也正是連線時序列化的單位 | 初階/未驗 | 「SideState 為什麼不做成獨立檔案的 class_name?什麼時候該升級?」 |
| 重構掃描的盲區:「函式改簽名」parse 期就叫、「屬性搬家」(mana 搬進 SideState)點取處要**執行到那一行**才炸——重構後 grep 的不只函式名,還有 `物件.舊屬性`;headless 啟動測試抓不到「條件路徑」上的錯(魔力不足才走到的那行) | 初階/未驗 | 「為什麼 battle_manager.mana 這個 bug 啟動時不炸、出牌被彈回時才炸?重構後該 grep 什麼?」 |
| 除錯器截停的長相:執行期錯誤發生時編輯器把你切出遊戲視窗、畫面像當機——「離開 app 且卡住」≠ 程式崩潰,是 debugger 停在錯誤行等你看;先去讀底部的錯誤訊息,別急著重開 | 初階/未驗 | 「遊戲視窗突然失焦卡住,第一件事該看哪裡?跟真的 crash 差在哪?」 |
| 「整批重建」的懸空參考家族:視圖重建(rebuild)會 free 舊節點,凡是還指著它們的參考(拖曳中的卡、hover 中的卡)都要在重建**前**清空——is_instance_valid 是最後保險,不是第一道門;列「誰會指著這批節點」的清單來掃,別靠碰運氣 | 初階/未驗 | 「拖著卡按結束回合,舊版會發生什麼?為什麼修法是先 organize_hand 而不是只加 is_instance_valid?」 |
| 動畫的「感」=距離×節奏:發牌感消失是因為重建把「單張從牌堆飛入」變成「原地生成」——修復=給距離(從牌堆出發)+給節奏(stagger 依序起飛);重構搬走一段流程時,附在上面的演出語彙要跟著搬 | 初階/未驗 | 「stagger 0.06 是什麼單位?為什麼平時重排用 0、只有發牌用 0.06?」 |

### 22. 連線 2a:大廳與 ENet 握手(2026-07-11;[net_lobby.gd](../src/net/net_lobby.gd)、[net_match.gd](../src/net/net_match.gd)、[main_menu.gd](../src/main_menu/main_menu.gd))

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| peer 掛在樹、不掛在場景:`multiplayer.multiplayer_peer` 屬於 SceneTree 的 MultiplayerAPI——換場景(大廳→牌桌)節點全死,**連線不斷**;所以大廳能「用完就丟」,回主選單賦 `null` 就回離線(還原成 OfflineMultiplayerPeer) | 初階/未驗 | 「大廳節點在 change_scene 時被 free 了,為什麼連線還在?誰負責在回主選單時斷線?」 |
| RPC 的路徑合約:RPC=「叫**對面同路徑節點**跑同名函式」,所以 NetLobby 的名字與掛點是合約的一部分;`@rpc("authority","call_local","reliable")` 三參數=誰有資格叫(host 權威,client 偽造被丟)/自己要不要也跑一份/掉包要不要重送 | 初階/未驗 | 「把 NetLobby 改名只改一台,握手會發生什麼?拿掉 call_local 會變成誰單獨開局?」 |
| 亂數只在 host(ADR-001 第一次落地):牌桌抽籤由 host 抽、把「結果」(index)塞進握手 RPC 廣播,client 只寫不抽——否則兩台各抽各的,你在森林他在冰原;之後的洗牌/抽牌全比照這條 | 初階/未驗 | 「為什麼不讓兩邊用同一個亂數種子各自抽?那種同步脆在哪?」 |
| 同進程雙分支測試:`SceneTree.set_multiplayer(api, root_path)` 能在一個進程開兩條 multiplayer 分支,host/client 走真 UDP loopback 互連——RPC 路徑以各自的 root_path 為基準解析,兩邊都叫 "NetLobby" 就對得上;連線功能因此能 headless 自動化驗證 | 初階/未驗 | 「verify 裡 host 的 NetLobby 在 /root/HostSide 下、client 的在 /root/ClientSide 下,RPC 為什麼找得到對方?」 |
| 繞開 class_name 快取(§19 的解法):實例化/引用**同批新增**的類別用 `preload("res://路徑")` 存進 const,不用 class_name——路徑載入不查全域類別快取,沒有「等編輯器重掃」的時間差;net_lobby 引 NetMatch、main_menu 引 NetLobby 都是這樣踩過才改的 | 初階/未驗 | 「main_menu.gd 為什麼用 NET_LOBBY_SCRIPT.new() 而不是 NetLobby.new()?什麼時候兩者等價?」 |

### 23. 法術卡面與素材管線(2026-07-11;[card.gd](../src/card/card.gd)、[data/cards/](../data/cards/)、assets/ui/icons/)

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| HD-2D 的特效真相:歧路旅人=2D 像素角色 × 3D 舞台 × **現代 3D 粒子特效+泛光後製**——特效不是像素圖!所以本作的法術特效有兩條路:GPUParticles3D+glow(歧路旅人感,WorldEnvironment 已開 glow)或 2D 幀動畫看板(復古感);卡面圖示是 UI 平面圖,跟場上特效是兩回事 | 初階/未驗 | 「歧路旅人的火焰魔法如果是像素幀動畫,畫面會少了什麼?我們的 glow_hdr_threshold=0.95 跟特效有什麼關係?」 |
| 後路分支的價值:setup() 的 `elif data.art != null` 當年寫著「理論上不會走到」,今天法術卡直接踩著它上線零改動——fallback 別只寫 push_error,把行為寫完整,未來功能常常就是免費的 | 初階/未驗 | 「如果當初 elif art 只寫 push_error('不該走到'),今天要多做哪些事?」 |
| AtlasTexture 切格子:一張大圖 + N 個 region 引用,不裁幾百個小檔——載入一次、記憶體一份、檔案系統乾淨;region 座標=格位 × 格尺寸;.tres 手寫格式三件套:ext_resource(path 引用)/sub_resource/load_steps=資源總數+1 | 初階/未驗 | 「16 顆圖示裁成 16 個 PNG 和一張大圖+16 個 AtlasTexture,差在哪?load_steps 數錯會怎樣?」 |
| 匯入系統的邊界:res:// 的圖要有 .import 才 load 得到(headless 同樣受限),編輯器取得焦點才會掃描匯入新檔;`Image.load_from_file` 繞過匯入直接讀原始檔(驗證未匯入素材的後門);`.gdignore` 讓資料夾不被掃描(原始 zip 的隔離區) | 初階/未驗 | 「為什麼我 F5 看得到新圖、你 headless 卻 load 失敗?.gdignore 和 .gitignore 各管什麼?」 |
| static 變數的寫入路徑:透過 class_name 寫 OK(`ArenaPool.next_arena_path = x`),透過 **preload 的 const 類別引用**寫會被編譯器當「改常數」擋下(讀常數/呼叫函式都行,唯獨賦值不行)——跨檔寫 static 收進該類別自己的 static 函式(`NetMatch.start_online()`),順便把「哪些欄位一起變」封裝成一個動作 | 初階/未驗 | 「NET_MATCH.PORT 讀得到、NET_MATCH.reset() 叫得動,為什麼 NET_MATCH.is_online = true 編譯不過?修法為什麼是加 start_online() 而不是把 const 改成 var?」 |
| 圖塞窗三策略:contain(`minf`,完整進窗**必留白**)/ cover(`maxf`,填滿**必裁切**,凸出靠遮罩吃 =§9 出血)/ fill-stretch(高度等比+`scale.x` 拉寬,填滿**必變形**)。三者只能挑一種代價,挑哪個是美術判斷不是對錯 | 初階(2026-07-11 教過+實測:留白方向答對、推出「畫大凸出」;cover 裁 30% 後**他自己打槍改要 fill**——能對策略代價做取捨了,快到中階) | 「窗改直向(0.9:1.3),cover 改裁哪邊?fill 的變形率怎麼算?什麼素材該用 contain 死也不變形?」 |
| VSCode 的 GDScript 紅字 ≠ 引擎判決:語言伺服器沒連 Godot 編輯器時認不得 class_name 全域型別(SkillData/CardType 整排假錯);地面真相用 `godot --headless --quit` 或編輯器本身驗 | 初階(2026-07-11 親歷:改 minf→maxf 後 IDE 爆 13 個假錯,headless 零錯誤) | 「IDE 說 SkillData 找不到、遊戲卻跑得動——兩個檢查器差在哪?要信誰、怎麼驗?」 |

### 24. 回合抽牌動畫:動畫跟隨資料變化量(2026-07-11;[player_hand.gd](../src/play_hand/player_hand.gd)、[card_manager.gd](../src/card_manager/card_manager.gd))

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 動畫該演「資料變了多少」:開局=整手都是新的→整手飛入(rebuild fly_in);回合抽牌=只多 1 張→只飛那張(deal_last_from_deck);換邊=整手換人但不是「發牌」→直接就位。用錯就是這次的 bug:換邊重演整手發牌,看起來像重新發牌 | 初階/未驗(逃生艙修復,根因他沒自己追) | 「_sync_hand_view 為什麼保留整手飛入、_on_end_turn 卻關掉?之後做『對手偷走你一張牌』,動畫該演什麼、資料變化量是多少?」 |
| 用「預設參數」擴充 API 不炸舊呼叫:_arrange_fan 加 instant、rebuild_from 加 fly_in,都給預設值 → 既有呼叫點一行不用改 | 初階/未驗 | 「如果 fly_in 不給預設值,專案裡哪一行會立刻爆?為什麼預設值要選 true 而不是 false?」 |

### 25. 連線視角鎖定與回合換頁 RPC(2026-07-11;[card_manager.gd](../src/card_manager/card_manager.gd)、[battle_manager.gd](../src/battle_manager/battle_manager.gd))

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 視角鎖定:熱座「視圖跟著行動方換頁」/連線「視圖鎖死自己那側」——同一份帳,兩種投影策略;判斷式收成一個函式(_hand_view_shows_active_side),讓「誰該看什麼」只寫一次 | 初階/未驗(逃生艙) | 「連線時輪到對方,我的手牌視圖為什麼一行都不用動?熱座為什麼反而要整手換頁?」 |
| RPC 換頁令:`@rpc("any_peer","call_local","reliable")`——any_peer 因為 client 也要能發令;call_local 讓發令方自己也執行;兩端各自收拾自己的拖曳/選單再翻頁(懸空參考防炸要兩端都做)。**呼叫端分流:連線才 `.rpc()`,離線本地直呼**(原因見下一列) | 初階/未驗 | 「為什麼這裡不能用 authority?對面機器上被拖著的卡誰去收?」 |
| `.rpc()` 需要活著的 peer:`multiplayer_peer = null` 時 rpc **報錯且整個不執行**(call_local 也救不了);**null ≠ OfflineMultiplayerPeer**。F6 直跑(預設 Offline peer)與走主選單(lobby 把 peer 清成 null)是環境不同的兩條進場路,bug 只在後者出現——測試要把所有進場路徑都走過 | 初階(2026-07-12 逃生艙自炸自修:headless 對照兩條路徑抓到;net_lobby 舊註解也錯在同一點,已更正) | 「同一段 code,為什麼 F6 直跑正常、從主選單進來按鈕就死?『離線直呼、連線才 rpc』的分流為什麼比『永遠 rpc』穩?」 |
| my_side 的離線預設值 = "player" 是刻意的:所有「連線 vs 熱座」的分支寫成跟 my_side 比較,離線語意自動退回熱座——預設值選得好,分支就少一半 | 初階/未驗 | 「side_name 那行為什麼離線時不用另寫一條?如果 my_side 離線預設是空字串會炸哪裡?」 |
| **2b 剩餘的債(還沒做,別誤以為完工)**:①兩台的牌堆各自亂洗(帳沒同步,上桌行動也還沒廣播)②發令資格只在 UI 閘把關,沒有伺服端驗證(client 可偽造換頁令)③client 的鏡頭/棋盤仍是 host 視角 | ——(待辦,非觀念) | 動工 2b 時逐項劃掉 |

### 26. 第一顆 3D 特效:通用命中爆點(2026-07-12;[src/fx/fx_burst.gd](../src/fx/fx_burst.gd))

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| GPUParticles3D 一次性爆點解剖:`one_shot + explosiveness=1`(爆點 vs 噴泉)、參數給 min/max 讓每顆粒子抽籤(不整齊才像火花)、`scale_curve` 要包成 CurveTexture(粒子系統只吃貼圖不吃函數)、additive blend(重疊更亮=火光感)、emission 3.0 推過 glow 門檻(泛光,和卡槽高亮同一招)、`finished → queue_free` 自毀 | 初階/未驗(逃生艙) | 「explosiveness 0 和 1 差在哪?想改成持續冒煙要動哪幾個參數?淡出為什麼用縮小而不是改 alpha?」 |
| 特效掛在「所有傷害的必經之路」(Card/Hero 的 take_damage):一次接線,普攻/反擊/技能/灼燒中毒全生效——找 choke point,別在每個事件各接一次 | 初階/未驗 | 「之後要加『治療綠光』,掛哪個函式?為什麼不掛在攻擊指令那端?」 |
| `current_scene` 在場景切換空窗期可能是 null;掛短命節點用 `get_tree().root` 更穩(headless 煙霧測試先抓到,沒等到玩家踩) | 初階/未驗 | 「特效掛 root 為什麼不怕跨場景殘留?什麼樣的節點反而不該掛 root?」 |

### 27. 法術結算層:四種卡型上線(2026-07-12;battle_manager / card_manager / battle_ui)

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| `await` 做「等玩家決定」:秘術流程走到反制窗口就 `await battle_ui.reaction_decided`,整條函式停在那行直到面板按鈕 emit——遊戲流程的「暫停點」不用狀態機硬刻,協程天生就會等 | 初階/未驗(逃生艙) | 「await 那行在等什麼?如果玩家永遠不按,函式怎麼了?為什麼進 async 前要先把 DRAGGING 狀態清掉?」 |
| 宣告即付費(§5.1 STEP1):先 spend 再開反制窗,被抵銷不退費——付費點的位置就是規則本身,放錯位置=規則錯 | 初階/未驗 | 「把 spend 移到結算成功之後,遊戲規則變成什麼樣?哪一方變強?」 |
| 靈裝加成記在「節點」(max_hp_bonus)不寫回共享的 CardData:寫回 data.hp 會讓全場同名卡一起變厚(=主帳 §7 Resource 共享的實戰版);宿主離場,節點亡,加成自然消失,不用寫清理 code | 初階/未驗 | 「為什麼 +2 上限不能直接 data.hp += 2?『裝備隨宿主離場』是誰保證的——有寫這段 code 嗎?」 |
| 伏印住在「側帳」不佔卡槽(§2 後排資料層);觸發點挑在 mark_summoned=召喚的必經之路——和爆點掛 take_damage 同一招:找 choke point,一次接線全情境生效 | 初階/未驗 | 「伏印為什麼不做成一張蓋在卡槽上的卡?觸發檢查為什麼放 mark_summoned 而不是放在出牌按鈕那邊?」 |
| 瞬咒候選查「帳」不查「視圖」:守方的手牌在熱座時已 stash 回 sides[defender].hand,視圖上根本沒有他的牌——帳/視圖分離下,規則永遠問帳 | 初階/未驗 | 「反制窗口跳出來時,守方的牌在哪個資料結構裡?為什麼不能去 player_hand.cards 找?」 |

### 28. 連線 2b–2e:帳同步+行動重放(2026-07-12;card_manager / battle_manager / battle_ui)

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 同步架構的最小可行式:「開局資料一致(host 打包牌堆/手牌給 client)+ 每個操作一致(行動 RPC 兩台重放)= 狀態一致」;驗收方式是**簽名比對**——兩台各印雙方手牌+牌堆剩量,必須一字不差 | 初階/未驗(逃生艙) | 「為什麼開局要同步牌堆而不是每次抽牌廣播結果?如果某台在重放時 lag 掉一個 RPC,簽名會發生什麼?」 |
| 跨機器身分證:節點參考不能過網路——單位=「所在卡槽的場景樹路徑」(兩台樹一致才成立)、本體=側別字串、手牌=索引(兩台帳一致才成立)。身分證的有效性建立在「某種東西兩邊一致」上 | 初階/未驗 | 「為什麼卡槽路徑可以當單位的 ID?什麼情況下手牌索引會失效?」 |
| 出牌端 vs 重放端的不對稱:出牌端有「真的被拖過來的卡節點」(_pending_play_card,動畫連續),重放端只有帳+索引(生新節點/扣帳面)——同一個 RPC 函式裡走兩條路,用 _acting_locally() 分流 | 初階/未驗 | 「_net_summon 裡為什麼出牌端不用 remove_from_hand?重放端為什麼不能用 _pending_play_card?」 |
| 反制窗口跨機器:宣告(兩台記 pending)→ 守方**那台**開面板 await 人類 → 把布林結果 RPC 回來兩台各自結算——await 等的是「另一台機器上的人」,流程凍結點在守方機,出牌端只鎖操作等結果 | 初階/未驗 | 「秘術宣告後出牌端在等什麼信號?resolve 為什麼兩台都要跑 consume_quick,不會重複扣嗎?」 |
| 視角翻轉:繞「通過棋盤中線的垂直軸」轉 180°(v → (-x, y, 2·mid−z)),只轉觀察者那一掛(鏡頭/手牌/牌堆)——**世界只有一份,視角有兩個**;寫死 "player"/"enemy" 的判斷(_is_valid_target)在 client 上敵我顛倒,一律改「相對施放者」 | 初階/未驗 | 「為什麼翻鏡頭不翻棋盤?client 的『我方單位』在哪個群組?_is_valid_target 原本會怎麼錯?」 |

### 29. 牌桌構圖改版:鏡像布局+相機角度預算+hover 預覽(2026-07-14;player_board / card_manager / card.gd / battle_ui / main.tscn)

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 相機構圖是「角度預算」不是試手氣:物體可見 ⇔ 它對相機的俯仰角落在 pitch±fov/2 內——敵本體頭頂 22.8° vs 畫面上緣 23.5°(pitch 46−fov半角 22.5)只差 0.7° 就是被裁;fov 45→50 直接買 2.5° 頭部空間;下緣打到地面的 z = camZ − camH/tan(pitch+fov/2) | 初階/未驗(逃生艙) | 「想在畫面上緣多留一個頭的空間,有哪三種參數可以動?各自的副作用是什麼?」 |
| 視覺任務也能自動化迭代:CLI 非 headless 跑遊戲 + 執行期覆寫 transform + 截 PNG 檢視 → 收斂後才把數值「烘」回 .tscn;和「基準值要快照」同族——實驗態(執行期覆寫)與定案態(場景檔)分開 | 初階/未驗 | 「為什麼不直接改 .tscn 反覆開編輯器看?harness 覆寫和烘進場景檔各自的角色是什麼?」 |
| 鏡像對稱的紅利:布局對稱於 z=0 之後,client 翻轉視角(§28)自動變成完美鏡像——同一組相機參數服務兩台;「敵方前排比後排更遠」的語意顛倒也被「row0 永遠貼中線、兩側背向中線生長」自然修掉 | 初階/未驗 | 「舊版敵方 front/back 為什麼顛倒?鏡像後 _board_mid_z 變多少、哪些下游跟著變?」 |
| Label3D 沒有 max_lines:截斷用「全形 1/半形 0.5」估寬(夠用就好,量測 API 依賴字型載入時機);**資訊分層**:小空間(卡面)給摘要+…,大面板(hover 預覽)給全文——不是把字塞下去,是把閱讀場景分開 | 初階/未驗 | 「為什麼不精確量測每行寬度?卡面 5 行的上限是怎麼從座標推出來的(-0.52 中心、名 -0.1、攻血 -0.9)?」 |
| hover 事件序陷阱兩則:扇形重疊時 enter(B) 可能先於 exit(A) → 收面板要驗「離開的正是預覽中那張」;來源卡被 free(換手牌/陣亡)不會發 unhover → _process 裡 is_instance_valid 兜底收面板 | 初階/未驗 | 「無條件在 unhover 收面板會出什麼視覺 bug?為什麼還需要 _process 那道保險?」 |
| 「三處手動對齊」常數是雷:stream_z 同時活在 Stream 節點(.tscn)、ground_generator、forest_scatter——改一處壞三處;這次三處一起動並讓註解互相指路,長期解是單一來源(誰讀誰) | 初階/未驗 | 「如果只把 Stream 節點移到 z=0、兩支腳本不動,畫面會壞成什麼樣?怎麼設計讓這個常數只寫一次?」 |
| 合成旋轉要「全鏈一起算」:手牌卡的最終朝向 = hand 節點旋轉 ∘ 每張卡自帶的 card_tilt_x(+55°)——只調 hand 到 −46° 時合成變 +9°(卡近乎躺平,看起來又立又扁);「面向玩家」的判準是**卡面法線仰角 ≈ 鏡頭視線仰角**(定案 hand −108°+55°=法線 53° vs 視線 51°) | 初階/未驗 | 「為什麼 hand 轉 −46° 卡反而像躺平?怎麼從『相機位置+手牌位置』算出 hand 該轉幾度?」 |
| hover 抬升的漂移 bug(**第一版「快照+旗標」被 Harvey 實測打臉,已更正**):hover/unhover 快速交錯時,歸位補間還在飛、旗標卻已清掉,重新快照抓到「抬到一半」的位置 → 越 hover 越往前。真解兩層:①基準由**資料源頭**寫入(PlayerHand 排扇形時 sync_hand_base),動畫永遠 tween 到「基準+固定偏移」的**絕對目標**,打幾次都收斂同一點;②同一屬性只留一條補間(開新的先 kill 舊的;拖曳跟手也寫 position,開拖同樣要 kill)。回歸測試:30 輪交錯 hover 斷言零漂移 | 初階(bug 由 Harvey 實測發現,修法是 AI 代工) | 「為什麼快照+旗標防不住交錯 hover?『tween 到絕對目標』和『tween 相對位移 -=HOVER_LIFT』在連打下差在哪?兩條 tween 同時寫 position 誰贏?」 |
| 卡片閃爍 = hover 的「演出」推翻了 hover 的「判定」:抬升把碰撞箱一起搬走 → 游標下一幀指不到 → exit→歸位→又 enter……enter/exit 迴圈。通解=**判定與演出分離**:視覺照抬,$Area3D 反向補償釘在扇形原位(歸位途中也要同步收回,否則碰撞箱「掃回」游標又觸發 enter);補償向量要跨座標系換算(手牌空間抬升 → 卡 local:純旋轉反轉 ÷ hover 後縮放)。headless 斷言:抬升 1.8 期間碰撞箱世界漂移 0.000。可遷移:任何「指到就會動」的 UI,判定幾何都不能跟著動 | 初階(現象與問法由 Harvey 提出,原理講解後逃生艙代工) | 「為什麼以前只放大 1.35 不閃、抬升 1.8 就閃?補償為什麼要除以縮放?下拉選單 hover 展開是同一族問題嗎?」 |

### 30. 單人 vs AI 模式:AI=另一個玩家+三模式判斷式(2026-07-15;match_mode / enemy_ai / opponent_hand / card_manager / main_menu)

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| AI 設計成「另一個會按按鈕的玩家」:不開後門、不直改帳,結算走**和連線重放同一條函式**(_net_summon/_net_action 的帳面路徑)——所有行為者(人類/遠端/AI)走同一條結算路,規則與帳自動一致,AI 天生不會作弊也不會漏掉伏印/反擊等結算 | 初階/未驗(逃生艙) | 「AI 召喚為什麼不直接呼叫 spawn_unit 就好?讓 AI 走重放路徑,伏印觸發是誰保證的?」 |
| 一個判斷式吃三種模式:「誰在看(_hand_view_shows_active_side)/誰在動(_acting_locally)」收斂成單一函式後,新增 VS_AI 只要各加一個分支——熱座=恆真、連線=比 my_side、單人=比 "player";分散寫在每個呼叫點的話,新模式要改十幾處 | 初階/未驗 | 「VS_AI 的 enemy 回合,_acting_locally 為什麼要回 false?這個 false 讓哪條路徑被選中?」 |
| 介面與決策者分離:反制窗口的面板是「問人類」的介面——守方是 AI 時開面板等於(a)把 AI 手牌攤給玩家(b)要玩家替 AI 按鈕,所以 VS_AI 直接自動決策不開面板 | 初階/未驗 | 「單人模式玩家丟秘術,反制窗口會出現在誰面前?為什麼一個都不該出現?」 |
| class_name 全域註冊表的雷:`.godot/global_script_class_cache.cfg` 由**編輯器掃描**生成——編輯器外新建帶 class_name 的檔,headless 直接跑會報 "Identifier not declared";`--headless --editor --quit` 跑一次重掃即解 | 初階(AI 自踩自修) | 「為什麼同一份 code 在編輯器裡跑得動、headless 卻說 MatchMode 未宣告?這個 cache 是誰在維護?」 |
| 回歸測試資產要落籍版控:系統暫存目錄會清檔(本專案兩度弄丟測試腳本),tests/ 進 repo 後「重跑驗收」才是零成本動作;測試斷言要挑「不受隨機性影響的不變量」(認召喚暈眩標記,不認卡名/場上總數——起手隨機、反擊會抵銷數量) | 初階/未驗 | 「為什麼『AI 召喚了』不能用場上單位數+1 驗?summoned_this_turn 為什麼在 AI 回合結束後還留著?」 |

### 31. 秘術施放改爐石式箭頭瞄準:重用狀態機分支(2026-07-15;[card_manager.gd](../src/card_manager/card_manager.gd))

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| **旗標區分共用狀態 vs 開新狀態**:秘術瞄準沒新增 `UiState.SPELL_TARGETING`,而是複用既有 `TARGETING`,靠 `pending_spell_card` 旗標分流(active_unit=單位攻擊、pending_spell_card=秘術)。因為 `_input`/`_process`/`_cancel_command`/`_on_game_over`/換頁 全都認 `ui_state` 分派——多開一個狀態就得在每個分派點各加一條分支;用旗標則所有共通清理(右鍵取消、換頁、勝負)自動涵蓋,不會漏 | 初階/未驗(逃生艙) | 「如果當初開 SPELL_TARGETING 新狀態,右鍵取消秘術瞄準要多改哪幾個地方才不會卡死?」 |
| **改互動不動結算**:只換了「怎麼選到目標」(拖放放開→點卡+箭頭+點目標),下游 `_resolve_arcana`/連線宣告 `_net_arcana_declare`/守方反制窗口一行沒改——目標一旦選定,結算跟「當初怎麼選的」無關。同一條「演出/輸入 vs 規則/結算」分離線,跟 §27 是同一招 | 初階/未驗 | 「把秘術從拖放改成箭頭,為什麼反制窗口與連線同步的 code 完全不用動?這條分界線畫在哪?」 |
| **箭頭起點是「施放者」的抽象**:`update_arrow` 本來就吃任意 `from_px`;把起點從「攻擊單位 active_unit」一般化成「施放者節點」(單位 or 本體),同一條箭頭渲染兩用。秘術的施放者=行動方本體(`active_side` 對應的 hero),不寫死 player_hero——熱座/連線 client 時行動方會換邊 | 初階/未驗 | 「秘術箭頭起點為什麼用 `active_side` 挑 hero,而不是永遠用 player_hero?哪種模式會露餡?」 |
| **秘術瞄準不能共用 `_is_valid_target`**:後者靠 `side_of(active_unit)` 判敵我,秘術時 active_unit=null 會炸;所以拆出 `_is_valid_spell_target`(敵方+非潛行)並用 `_is_valid_targeting_target` 依模式分派。**基準值(施放者)換了來源,判定函式就得跟著換** | 初階/未驗 | 「秘術瞄準時 active_unit 是什麼?若硬套舊的 _is_valid_target 會在哪一行爆、為什麼?」 |

### 32. 景深(DOF):HD-2D 微縮感+透明物件的深度陷阱(2026-07-15;main.tscn / hero / card / opponent_hand / card_manager)

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| DOF 是「相機的後製」不是材質效果:引擎拿**深度緩衝**逐像素查「離相機多遠」,對焦範圍外就糊——所以素材本身不用改,但需要 Forward+(本專案已是)且每個物體有把深度寫進緩衝。掛在 Camera3D 的 CameraAttributesPractical(far 20/transition 5/amount 0.08;距離沿視線方向量:手牌 4.8、戰區 13.6、敵本體 18.5、森林 21+,遊戲資訊全在對焦區內);near blur 刻意不開——手牌深度 4.8 離鏡頭最近,一開最先糊的就是手牌 | 初階/未驗(逃生艙;觀念先教過一輪) | 「DOF 為什麼不用改素材?far_distance 20 這個數字是怎麼從場景量出來的?為什麼 near blur 對卡牌遊戲是自殺?」 |
| 透明物件是「靠深度的後製」的頭號嫌犯:Sprite3D/Label3D 預設**不寫深度**,DOF 讀到的是它「身後森林」的深度 → 站在對焦區的立牌被當遠景糊掉。修法=`ALPHA_CUT_OPAQUE_PREPASS` 讓不透明像素寫深度(像素硬邊零損失);同族效應也適用 SSAO/SSR/霧 | 初階/未驗 | 「為什麼立牌明明站在對焦區卻被糊?alpha_cut 改了什麼?哪些後製效果會有同一族問題?」 |
| Label3D 掛 alpha_cut 的反坑:字身/外框是兩層**共面**的面,平常靠 render_priority(只在透明管線有效)排序——進了深度管線排序失效,黑外框蓋掉白字身(截圖抓到「HP 20」變黑字)。真解=**深度錨定板**:字後面墊一塊會寫深度的不透明底板,字的像素被釘在底板的對焦距離,字本身維持透明管線;順便換到可讀性紅利(白字不再直壓雜亂背景) | 初階/未驗 | 「為什麼 Sprite3D 掛 alpha_cut 沒事、Label3D 掛就黑字?底板為什麼能讓『沒寫深度的字』不被糊?」 |
| 「想過才不做」兩則:敵方牌堆數字在過渡帶會微糊——但它和周圍地景**同深度同糊法**,視覺一致,不值得為它加板;飄浮傷害數字 0.8 秒即逝且帶淡出動畫(alpha_cut 與淡出的互動未驗),不動 | 初階/未驗 | 「牌堆數字微糊為什麼可接受、本體 HP 糊掉不可接受?判準是什麼?」 |

### 32. 異地連線:UPnP 打洞+執行緒輪詢(2026-07-16;net_upnp / net_lobby)

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 區網 IP vs 對外 IP:家用機器只有 192.168.*,「對外 IP」是**路由器的**;朋友連對外 IP 時封包到路由器就停(NAT 不知道轉給誰)——所以「顯示浮動 IP」只做一半,還要**打洞**(端口轉發)。UPnP = 程式對路由器說「UDP 8910 進來都轉給我」;三方案取捨:UPnP 自動(零設定,路由器要支援)/手動轉發(萬用,要進路由器後台)/Tailscale(組虛擬區網,雙方都要裝) | 初階/未驗(逃生艙;NAT 觀念先教過一輪) | 「為什麼只把對外 IP 顯示給朋友還不夠?ENet 走 UDP,打 TCP 的洞會怎樣?CGNAT 是什麼、為什麼打洞無效?」 |
| 阻塞操作進 Thread+官方輪詢模式:discover() **標稱 2 秒、實測多網卡 Mac 上 ~10 秒**(標稱值≠實測值,逾時上限要用實測定)——擋主執行緒會凍住整個選單;結果回收走 is_alive() 輪詢 + wait_to_finish() 取回傳值,比執行緒裡 call_deferred 回打好推理(回呼送達時機取決於佇列沖刷,輪詢每幀確定檢查)。第一版探針等 8 秒就宣告失敗,其實是 discover 還沒返回——量測不足就下結論的實例 | 初階/未驗 | 「為什麼 discover 一定要開 Thread?輪詢和回呼各自的送達保證差在哪?探針第一版誤判的根因是什麼?」 |
| 「洞是路由器的狀態」:生命週期不隨遊戲進程/場景——大廳節點換場景就死,但對局中洞要留著 → static 列管(仿 ArenaPool),回主選單由新大廳 _ready 收殘洞;慢半拍回來的結果要先驗「時過境遷」(開洞期間玩家已取消開房 → 直接拆) | 初階/未驗 | 「為什麼 UPnP 狀態不能放在 NetLobby 的成員變數?開洞回來發現 multiplayer_peer 是 null,該做什麼、為什麼?」 |
| 探針測試 vs 回歸測試:結果**依環境而異**的測試(UPnP 看路由器臉色)不能進 CI 當斷言,定位是「換網路先跑一次的體檢工具」;本機只驗得了 fallback 路徑,成功路徑要到有 UPnP 的網路才算數——測試的「綠」要誠實標注覆蓋範圍 | 初階/未驗 | 「upnp_probe 和 spell_smoke 的『過』意義有何不同?為什麼不能把『打洞成功』寫成斷言?」 |

### 33. 素材重構:引用熱度分級+路徑合約(2026-07-16;assets/ 全域、arena_*.gd、data/cards)

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 重構素材前先量「引用熱度」:grep 出誰引用誰再動手——四個素材包=**單點**常數引用(各改 1 行)、卡牌立牌=24 個 .tres 批次改寫、terrain/psx_trees=89+68 引用的熱區(已合規→**不動**,尤其 meshlib 靠文字路徑 fallback 活著);執行順序=先刪零引用(零風險)→再搬→再改寫→重掃驗證 | 初階/未驗(逃生艙) | 「為什麼先刪再搬?terrain 資料夾也想收進 packs/ 的話,代價是什麼、值得嗎?」 |
| 這個 repo 的「真合約」是文字路徑不是 UID:`*.import` 被 gitignore → UID 每台機器重生、跨機器不可信(grasslands meshlib 的 UID 警告就是活證據)——所以搬移後改寫 .tres/.gd 的路徑字串是**必要**,不是保險;`git mv` 整資料夾時磁碟上的未追蹤 .import 會跟著走,本機 UID 不斷 | 初階/未驗 | 「如果 .import 有進版控,搬移的風險會變高還是低?為什麼這專案的 .tres 在別台機器會印 UID 警告但仍能載入?」 |
| perl `-CSD` 的 UTF-8 陷阱:`-e` 腳本裡的中文 pattern 是**位元組**、`-CSD` 把檔案內容解碼成**字元**——兩邊型別不合 → 靜默零匹配;救命的是「批次改寫必配批次驗證」(改完 grep 新舊字串計數對帳),而不是相信指令跑完沒報錯 | 初階/未驗 | 「為什麼 perl 沒報錯卻一個都沒改?如果沒有做改寫後的 grep 對帳,這個 bug 會在哪一刻爆?」 |
| 素材衛生的量級感:匯出預設「打包全部資源」→ 零引用的 82M(未用字重 73M+參考圖 8M+殘複本)全在 zip 裡陪跑;`.gdignore` 讓資料夾完全不被引擎掃描(docs/ 加一個,參考圖搬進去就不進匯出);「想過才不做」×3:包內結構不改名(對照授權/更新)、熱區不動、.gd 註解保留原始產品名 | 初階/未驗 | 「刪掉 73M 字重後,匯出 zip 會小多少?為什麼參考圖搬 docs/ 還要加 .gdignore?」 |

### 34. 墓地+丟牌回魔:單一入口與冷卻的 off-by-one(2026-07-16;battle_manager / grave_pile / card_manager)

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 「離場」只給一個入口:所有離場的牌(死亡+隨葬靈裝、用畢秘術/瞬咒、觸發伏印、爆牌、棄牌)統一走 `bury()`——和爆點掛 take_damage、伏印掛 mark_summoned 同一招 choke point;且埋點全在**兩台會重放的函式**裡,連線因此**零新增同步**(資料一致+操作一致=墓地一致,§28 的複利) | 初階/未驗(逃生艙) | 「為什麼墓地不需要新的 RPC?如果在 UI 層直接 append 進 grave 陣列,連線會出什麼事?」 |
| 冷卻的 off-by-one:規格「用後下一回合不可再用」→ 冷卻要設 **2** 並在**自己**回合開始遞減——直觀的「設 1」會在下一個自己回合就歸零,變成每回合都能用;規格草稿自己的虛擬碼就是錯的(已修 README §1.1),**規格文字與規格虛擬碼打架時,以文字語意為準並回頭修文件** | 初階/未驗 | 「設 1 和設 2 的行為差在哪一回合?如果改成『對手回合也遞減』,節奏又會變成什麼?」 |
| Node vs Resource 的生命週期:`queue_free()` 殺的是**節點**,`card.data`(Resource)有人引用就活著——所以牌進墓地不用複製資料,墓地/牌堆/手牌存的都是同一份 CardData 的參考(§7 Resource 共享的另一面:這次是紅利不是坑) | 初階/未驗 | 「秘術節點 free 掉之後,墓地裡那張卡的名字為什麼還讀得到?什麼情況下這種共享會變回坑?」 |
| 隨葬清單記在宿主節點:靈裝的「加成數值」(max_hp_bonus)和「卡本體」(equipped_cards)分開記——數值天天用、卡只在陣亡時用一次;宿主 free、清單跟著消失,不用寫清理 | 初階/未驗 | 「為什麼不把 equipped_cards 記在 BattleManager 的字典裡(unit→cards)?那樣要多寫什麼?」 |
| 測試要挑「有牙齒」的輸入:第一版棄 hand[0] 抽到 cost 1 → 回魔 0,斷言「通過」但等於沒驗(0+0=0);改挑手上最貴的牌才驗到真加魔。**退化案例會讓綠燈說謊** | 初階/未驗 | 「為什麼 cost 1 的棄牌測試是『假綠燈』?還有哪些斷言可能因輸入退化而空轉?」 |
| 視覺擺位也要查「場地約束」:墓座第一版放牌堆與中線之間,正好踩進溪流水帶(±1.7,§29 的岸線保證)——挪 x 不挪 z 才躲開;新視覺物件上場前,先問「這個位置有什麼既有的東西」 | 初階/未驗 | 「為什麼墓座往中線挪會進水?場上還有哪些『看不見的保留區』?」 |

### 35. 瞄準點空取消(爐石式)+墓地入土演出(2026-07-16;card_manager / grave_pile)

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 取消要走**同一個出口**:`_cancel_command()` 是瞄準/選單唯一的反悔出口(收箭頭高亮、清 pending_spell_card/pending_skill/active_unit、關 UI 一次到位);「點空白取消」只是**多一個入口**呼叫同一個出口,右鍵/ESC 照舊。若每個入口自己清狀態,漏一項就是殘留 bug(箭頭掛著、旗標沒清、下一次點擊行為錯亂) | 初階/未驗(逃生艙) | 「點空地取消為什麼不能只寫 `ui_state = IDLE`?會漏掉哪三件事?各漏掉會出什麼症狀?」 |
| **射線打得到 ≠ 語意上合法**:raycast 同層也打得到「手牌卡」,而 `side_of()` 對無槽卡回 `""`——舊碼點擊處理沒驗 `is_on_board`,`"" != active_side` 讓自己手牌被誤判成「敵方合法目標」(秘術可砸手牌的潛伏 bug,這次順手堵掉)。可遷移:射線只回答「打到什麼」,合法性(在場上?敵我?狀態?)要另外驗完整 | 初階/未驗(逃生艙) | 「為什麼 side_of 回空字串反而讓非法目標『通過』敵我檢查?這類『例外值撞上不等式』的 bug 怎麼防?」 |
| 負面回饋兩級制:點錯「東西」(本體/自己人/潛行)→ 講原因再取消;點「空地/手牌」→ 安靜取消——玩家點空地的最可能意圖就是收手,再彈訊息是噪音。UX 通則:**回饋強度對齊玩家意圖的最可能解讀**,不是對齊系統內部發生了什麼 | 初階/未驗 | 「同樣是取消,為什麼點本體有訊息、點空地沒有?如果一律彈『已取消』會差在哪?」 |
| emission 脈衝 = 事件的視覺驚嘆號:邊框平時 GLOW_IDLE 0.7(靜態標位置)、入土瞬間跳 3.5 再 0.7s 衰減——超過 glow_hdr_threshold 0.95 的亮度被 WorldEnvironment 泛光放大(FxBurst/卡槽高亮同一招)。通用節奏:**狀態用低亮度常駐、事件用高亮度脈衝**;暗底+發光邊框比「把整塊調亮」更能在暗色地面上讀出形狀 | 初階/未驗(逃生艙) | 「脈衝為什麼是瞬間跳到峰值再慢慢衰減,而不是漸亮漸暗?邊框發光為什麼不吃場景光照也看得見?」 |
| 演出與帳的同步線:`shown_count`/張數標籤在 refresh() 開頭**同步**寫(headless 斷言不必等補間),落牌/脈衝/彈跳全是後續純演出;同幀連埋(陣亡+隨葬靈裝連呼 refresh)要先 kill 舊 drop tween 再開新的——§29「同一屬性只留一條補間」的新現場 | 初階/未驗(逃生艙) | 「如果 shown_count 改成落牌動畫結束才更新,grave_test 哪條斷言會 flaky?為什麼是 flaky 而不是必掛?」 |

### 36. 暫時魔力黃色顯示:帳中帳+信號簽名擴充(2026-07-16;battle_manager / battle_ui)

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 「帳中帳」:`temp_mana` 不是新規則,是把既有行為(回合開始回滿=暫時魔力自然蒸發)**標注**出來給 UI 上色——增減永遠跟著 `mana` 走、不變量 temp ≤ mana、回合開始歸零;規則一行沒變,變的只是「帳能不能說出這幾點的身世」 | 初階/未驗(逃生艙) | 「temp_mana 歸零寫在 _begin_side_turn 而不是回合結束,差在哪?對方回合打瞬咒還能用上回合的暫時魔力嗎?」 |
| 改信號簽名 = 先盤點訂閱者:signal 連接是**執行期**綁定,參數對不上不會編譯期報錯、發射當下才炸——動手前先 grep `state_changed.connect` 盤出全部訂閱者(本案只有一條:card_manager → update_hud),宣告/emit/handler 三處一次改齊 | 初階/未驗(逃生艙) | 「如果漏改 update_hud 的簽名,炸在哪一刻、錯誤訊息長什麼樣?為什麼編譯期抓不到?」 |
| 扣費收斂單一出口 `_pay()`:三個扣費點(召喚/瞬咒/技能)原本各自 `mana -= cost`,新增 temp 不變量後若各自維護,漏一處就是顯示錯帳——和 bury()/take_damage 同一招 choke point;「暫時的先扣」機制上無差(下回合全洗),挑它是讓**視覺語意成立**(黃◆疊最右、花費從右扣) | 初階/未驗(逃生艙) | 「先扣正常魔力的話,畫面上會出現什麼奇怪狀態?為什麼說兩種扣法機制上等價?」 |
| RichTextLabel 的尺寸坑:`fit_content` 只管**高度**,它不會用內容撐開最小**寬度**——而 HUD 面板的寬度正是靠魔力列撐的(置中約定),換它會讓長魔力列被裁/換行;改用 HBox+多顆單色 Label,寬度由 Label 的天生 min size 加總。可遷移:**挑 UI 節點先問「誰決定尺寸」** | 初階/未驗(逃生艙) | 「為什麼 Label 排版正常、RichTextLabel 會縮?如果硬要用 RichTextLabel,還缺哪一步?」 |
| 連線零同步紅利再現:`apply_discard_for_mana`/`_pay` 全在兩台會重放的函式裡跑,`temp_mana` 兩台天生一致,不加任何 RPC(§28/§34 的同一條複利);顏色資訊走既有 state_changed 順風車,不開新信號 | 初階/未驗 | 「如果 temp_mana 只在出牌那台更新,對手螢幕上會看到什麼?哪一條既有機制保證了它不會發生?」 |

### 37. 音效系統上線:一次性播放器+choke point 接線(2026-07-16;src/fx/sfx.gd + 8 檔掛載)

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 一次性節點播放器(同 FxBurst 的模式):每次 play 現場組 AudioStreamPlayer **掛樹根**、`finished → queue_free` 自毀——發聲者(卡片)可能下一幀就被 free,聲音的生命週期必須獨立於觸發者;重疊觸發各開各的節點,天生不互搶。headless(Dummy 音訊驅動)照跑不炸,測試零改動 | 初階/未驗(逃生艙) | 「音效播放器為什麼不掛在卡片身上?如果掛了,秘術用掉那張卡的施放聲會發生什麼?」 |
| pitch 微抖 ±6% 去機關槍感:同一顆 .ogg 連續觸發(連抽、連擊)聽起來像機器複讀——每次播放 `pitch_scale = randf_range(0.94, 1.06)` 就有「每次都不太一樣」的錯覺,比準備 5 個變體檔便宜得多(Kenney 包附的 place-1~4 變體反而不用全搬) | 初階/未驗 | 「為什麼調 pitch 而不是調 volume?抖動範圍開到 ±20% 會聽起來像什麼?」 |
| 掛載點選 choke point,一行接全部:入槽聲掛 `place_card`(本機出牌/連線重放/AI 召喚全過這裡)、受擊聲掛 `take_damage`(與命中爆點同點,普攻/反擊/技能/伏印全蓋)、點擊聲掛**按鈕建構器** `_make_option`(選單/反制/勝負畫面一次接上)——和 bury()/FxBurst 是同一門手藝:找「所有路都會經過的窄口」 | 初階/未驗(逃生艙) | 「入槽聲如果掛在 _try_summon,哪兩種召喚會沒聲音?點擊聲掛建構器 vs 逐顆按鈕接,差在哪?」 |
| @tool 腳本的副作用防護:player_hand 在**編輯器裡**也會跑 rebuild_from——出聲防護不寫在每個呼叫點,寫在 `Sfx.play` 開頭 `Engine.is_editor_hint()` 一鍋端(防護放 API 入口,不放呼叫點,漏不掉) | 初階/未驗 | 「如果防護寫在 player_hand 而不是 Sfx 裡,下一個在 @tool 腳本呼叫 Sfx 的人會發生什麼?」 |
| 素材「按用抽取」不整包解壓:只抽 12 檔(~136KB)落地 assets/audio/sfx,檔名改成**語意名**(hit/click/turn_flip,不是 impactSoft_heavy_000)——程式讀語意、LICENSE.txt 留對照表記出處;其餘幾百個音效留在 .gdignore 的 zip 裡(不進匯出、不佔掃描),要新音效再回去挑(§33 死重教訓的正向應用) | 初階/未驗(逃生艙) | 「為什麼落地檔名不保留原始檔名?對照表寫在 LICENSE 裡是給誰看的?整包解壓的代價是什麼?」 |
| 新 class_name+新資源都要過編輯器掃描才能 headless:`--headless --editor --quit` 一次做兩件事——.ogg 進匯入快取、Sfx 進 global_script_class_cache(§30 同款雷的預防性排除;.godot/ 不進版控,新機器第一次跑測試前也要做) | 初階/未驗 | 「新增的 .ogg 沒跑過編輯器,headless load() 會怎樣?這和 class_name 快取是同一個機制嗎?」 |

### 38. 伏印宿主制+裝備替換+三標籤標示(2026-07-17;battle_manager / card_slot / slot_tile.gdshader / card_manager)

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 動手前先盤點存量:「技能三標籤」原以為要做機制,結果 SkillData.kind 的**資料和結算層早就都在**(§6 藍圖時代備好的),真正缺的只有三處顯示——任務從「實作系統」縮成「標出來」。可遷移:接到需求先 grep 資料層,別急著開新欄位 | 初階/未驗(逃生艙) | 「kind 的行動經濟結算在哪個函式?如果當初資料層沒備好,這次要多做哪幾層?」 |
| 威懾=刻意的資訊不對稱:伏印埋設後亮的是**整排**(前後排全亮)而不是宿主那格——視覺洩漏位置,威懾就死了;同理 flash 訊息不能報宿主名(兩台都會看到)。「要給對手看什麼」和「要瞞對手什麼」都是規則的一部分 | 初階/未驗 | 「為什麼後排也要亮?訊息寫『伏印埋在騎士底下』會壞掉什麼?熱座模式這個威懾還成立嗎?」 |
| 帳跟宿主走:set_ward 的側別由 side_of(host) 推、不看 active_side;宿主陣亡時伏印/靈裝走同一條隨葬路(_check_death 統一 bury)——「附著物的生命週期=宿主的生命週期」寫在一個地方,不用每種附著物各記一次 | 初階/未驗 | 「如果 set_ward 用 active_side 記帳,哪個模式會埋錯邊?隨葬為什麼放 _check_death 不放 CardSlot?」 |
| shader 呼吸的分工:**顏色定紅、亮度呼吸**——第一版把顏色也乘呼吸相位,青紅各半混成灰粉色,「警戒」讀不出來;週期用 shader 裡的 TIME 做(恆定、不怕 tween 疊加),腳本只推開關的淡入淡出 | 初階/未驗(截圖抓到翻車) | 「顏色乘呼吸為什麼會變灰粉?呼吸放 shader 和放 tween 各有什麼取捨?」 |
| 裝備替換的結算順序:拆舊(加成收回)→ clamp 現血 → 穿新 → heal——順序錯了血量會多算或少算(6/6+2 穿新 +2:先拆夾回 6 再補 2 = 8/8);「上限變動」和「現血變動」是兩筆帳,各自有自己的時點 | 初階/未驗 | 「如果先穿新再拆舊,血量會變多少?clamp_hp 為什麼是 Card 的方法而不是 BattleManager 的?」 |
| class cache 之雷第三發:pull 進來的新 class_name(Sfx)不在本機快取 → 所有引用它的腳本**連鎖編譯失敗**,節點掉回裸基底型別(CardSlot 變 Area3D、is_empty 消失)——錯誤訊息離根因很遠;「換機器/pull 後先 --headless --editor --quit」升格為標準動作 | 初階(AI 自踩自修,第三次) | 「為什麼錯誤訊息是 is_empty 不存在而不是 Sfx 不存在?這顆雷為什麼 git 防不了?」 |

### 39. 墓地黑洞化+回魔雙演出+樹池迴圈化(2026-07-18;grave_pile / grave_vortex.gdshader / card_manager / forest_scatter)

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| 專案第一個 spatial shader:fragment 對每個像素跑一次,UV(0~1)→ 極座標(r=離心距、ang=角度)就能畫圓形圖樣;`TIME` 內建秒數讓動畫在 GPU 上白拿——**常駐小動畫放 shader(GPU、零腳本)vs `_process` 轉節點(CPU、每幀吃主執行緒)**,成本歸屬完全不同 | 初階/未驗(逃生艙) | 「黑洞自轉為什麼不用 _process 每幀 rotate?如果場上有 50 個這種裝飾,兩種做法的成本差在哪?」 |
| 相位正負號決定「往內吸 vs 往外噴」:螺旋亮紋 = 等相線 P = 臂數·ang + k·log(r) + w·TIME,固定角度下 log(r) = (P−…−w·t)/k → 時間越走 r 越小 = 紋路向圓心爬;當初寫成 −TIME 就默默往外漂,「在轉」但沒有「在吸」(Harvey 目視抓到的 bug) | 初階/未驗(逃生艙) | 「想讓紋路改成從中心往外湧(例如治療泉),要動哪個符號?內流環 sin(r·16+w·t) 的流向怎麼從式子看出來?」 |
| 雙材質拆分:**會被遊戲邏輯戳的**(吸積環 StandardMaterial3D:脈衝/轉金/提示)和**純裝飾自轉的**(黑洞盤 ShaderMaterial)分成兩份——黑洞化重寫整個外觀,對外 API(show_recycle_hint/arm_recycle)一字未變,呼叫端零改動 | 初階/未驗(逃生艙) | 「如果脈衝也做在 shader 上,card_manager 要多學會什麼?拆兩份材質換到了什麼?」 |
| preload vs load:preload 是**編譯期**指令、只吃字面路徑字串;load 是**執行期**函式、可用 `"tree%02d" % i` 組路徑——檔名有規律的資源池用迴圈 load 組(樹 36+灌木 8),別手抄 72 行;代價是載入時機移到執行到那行時,所以包進「只跑一次」的 _ensure_pools() | 初階/未驗(逃生艙) | 「preload 為什麼不能接變數?_ensure_pools 如果不做空池檢查、每次 _scatter 都重載,量級估一下會怎樣?」 |
| 狀態清理放單一收斂點:回魔提示的「熄滅」不寫在出牌/棄牌/取消每條退出路徑,而是 _process 開頭一句「沒在拖曳但提示還亮 → 熄」——之後新增退出路徑也漏不掉(和 §29 tween 互殺同族:與其記住所有入口,不如守住一個出口) | 初階/未驗(逃生艙) | 「這招的代價是什麼(每幀多一次 null 檢查)?什麼情況下反而該寫在退出路徑上?」 |
| 先上膛再結算:arm_recycle() 在 apply_discard_for_mana **之前**呼叫——bury 信號在 apply 內同步觸發 refresh,落地回呼雖是 0.3 秒後的補間才跑,但**正確性不該依賴「補間至少多長」這種巧合**;一個 flag + 落地分支,讓同一條 bury 管線長出金/紫兩種演出,不用開第二條管線 | 初階/未驗(逃生艙) | 「如果 arm 寫在 apply 之後,現在會壞嗎?哪天改成『入土演出瞬間完成』就會壞——為什麼?」 |

### 40. 墓地星雲重工:1mm 深度差 bug + 四層星雲 shader(2026-07-19;grave_pile / grave_vortex.gdshader)

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| **深度測試不同情 1mm**:漩渦盤 y=0.02,冰原湖面是圓柱(以中心定位:0.001 + 高 0.04/2 = 頂面 **0.021**)→ 盤整片被蓋掉;Torus 環縱跨 0.015~0.045、半截埋冰裡 → 只剩細細一圈。森林地板 y=0 所以看得到。可遷移原則:**場地是抽籤換的,貼地裝飾的高度要對「全池最高的地板」負責,不是對預設場地負責**(→ 收斂成 grave_pile 的 `BASE_Y=0.06`) | 初階/未驗(逃生艙) | 「為什麼環看得到、盤看不到?圓柱 position.y=0.001 時頂面在哪?如果之後新場地地板墊到 0.08,哪些東西會消失?」 |
| **對照實驗砍嫌疑犯**:F6 直跑 main.tscn(`next_arena_path` 空字串 → 用烤死的森林)vs 從主選單進(抽籤)——一次實驗就把「程式自己錯」和「場地相依」切開,嫌疑犯砍半 | 初階/未驗(逃生艙) | 「下次某個東西『有時看得到有時看不到』,你的第一個對照實驗是什麼?控制的變因是哪個?」 |
| **星雲四層合成**:雲霧底(fbm,亮度壓 glow 門檻下)/ 亮線條(sin 高次方壓細 + 噪聲截段,過門檻)/ 光點(格子擲骰,過門檻)/ 視界亮環(最亮)——**用亮度分層讓 glow 門檻當「圖層篩子」**:同一張圖,暗的當底、亮的自動泛光 | 初階/未驗(逃生艙) | 「想讓雲霧也泛光要調哪個值?為什麼 pow(sin, 10) 會把寬條紋壓成細線?」 |
| **無接縫旋渦噪聲**:極座標直接採噪聲,角度軸在 ±π 有裂縫;改成「直角座標旋轉 tw = k·log(r) + w·t 後再採 fbm」→ 天生無縫且越內圈扭越多;光點格子則用「整數 24 格 + mod」讓接縫兩側 hash 一致 | 初階/未驗(逃生艙) | 「為什麼 arm_count 要整數?如果角度格子切 24.5 格,畫面上會在哪裡出現什麼?」 |
| **hash → 值噪聲 → fbm 三件套**:不吃貼圖的程序化雲。hash=偽隨機、vnoise=格點間圓滑內插、fbm=多八度疊加(大雲塊上疊小碎絮) | 初階/未驗(逃生艙) | 「fbm 的 4 個八度各負責什麼視覺尺度?砍到 2 個八度雲會變怎樣?」 |

### 41. 抽濾系統:三張秘術+選牌面板+帳視圖對帳點(2026-07-19;skill_data / battle_manager / battle_ui / card_manager / data/cards)

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| **帳與視圖的「對帳點」**:行動方那台的手牌「帳」在回合中是舊的(出牌只移視圖節點,帳到 end_turn 的 stash_hand 才對齊)——法術抽牌動帳前要先 `stash_hand(player_hand.hand_data())` 對帳,否則滿手燒牌判定兩台不一致 → 一台燒一台入手,帳散掉 | 初階/未驗(逃生艙) | 「為什麼回合抽牌不用先 stash?(end_turn 換邊三步的第①步本來就是 stash)如果 _apply_draw 忘了 stash,具體會在哪個情境炸?」 |
| **「選擇」的連線同步**:牌堆兩台完全同步 → 選牌只廣播「第幾張」(索引),不廣播卡本身;選牌面板只開在行動方那台,對面看提示文字 | 初階/未驗(逃生艙) | 「如果改廣播卡的 resource_path 而非索引,哪裡會出賣手牌隱私?哪裡會在同名卡×3 時選錯張?」 |
| **付費後的選擇是義務**:選牌面板無取消鈕 + `_picking` 旗標擋右鍵取消/結束回合——不擋就變「付 1 費免費偷看頂三張再反悔」(同爐石「發現」不可取消) | 初階/未驗(逃生艙) | 「_picking 擋在 _on_end_turn(入口)而不是 _net_end_turn(結算),為什麼擋入口就夠?連線時對面按結束回合會怎樣?」 |
| **效果落地的時序**:bury/牌離手在前、效果(面板/抽牌動畫)在後——施法那張已離手才數手牌(滿手判定才對),畫面也乾淨 | 初階/未驗(逃生艙) | 「若效果先落地、牌後離手,抽到第 8 張時會發生什麼?」 |
| **enum 只能尾巴加 + 單一入口複用**:Effect 加 DRAW/SCRY/DISCARD_DRAW 只能 append(.tres 存 int,§21 同族);end_turn 的抽牌段重構成呼叫 draw_cards()——爆牌規則只寫一份(§20 choke point 同門) | 初階/未驗(逃生艙) | 「把 DRAW 插在 HEAL 前面,現有 24 張 .tres 會發生什麼?為什麼是默默壞不是報錯?」 |

### 42. 專用伺服器(ADR-002 第一刀):配對+權威換回合(2026-08-05;src/server/ 新資料夾、net_client / net_match / card_manager / main_menu / main_scene)

> **背景**:朋友實測連不上 → ADR-001 第 2 節第 4 條(區網/手動轉發/UPnP,公開配對 YAGNI)
> 被實測推翻。權威從「玩家開房」搬到 VPS 上的 headless Godot 進程。
> **這一節整批是逃生艙代工的,Harvey 一行沒寫。**關卡表 關 12(連線同步三件套)、
> 關 14(NAT/UPnP)維持 `未開始`——代工不算會。

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| **穿透問題要換「誰是發起方」,不是把洞打得更好**:NAT 只對**出向**連線自動建對應表,入向封包到路由器不知道轉給誰就丟掉。P2P 的死穴是總得有人接受入向(CGNAT 後面連打洞都無效,洞在電信商的路由器上)。專用伺服器的解法是**讓沒有人需要被連進來**——兩邊都往外連同一個公開 IP | 初階/未驗(逃生艙) | 「為什麼 VPS 方案不需要 UPnP?如果兩位玩家都在 CGNAT 後面,VPS 方案還成立嗎?為什麼?」 |
| **專用伺服器 = 一個沒有玩家坐著的 host**:BattleManager 本來就設計成「一方持有兩側的帳」,所以伺服器不是新東西,是把原本的 host 抽出來。關鍵一手是 `my_side = ""`——它讓 `_acting_locally()` 恆為 false,所有行動自動走「重放端」(從帳生節點),因為伺服器沒有人在拖牌 | 初階/未驗(逃生艙) | 「如果伺服器的 my_side 設成 'player' 會發生什麼?(提示:_take_pending_card 回 null)為什麼空字串比新加一個 is_server 判斷式好?」 |
| **MultiplayerAPI 分支與 root_path**:RPC 路徑是相對**各自的 root_path** 解析的,所以伺服器的 `/root/Room_0/MainScene/CardManger` 和玩家的 `/root/MainScene/CardManger` 對得上。⚠ 但**不能在已配置分支的子路徑再設一條分支**(房間本來掛在 GameServer 底下 → `Multiplayer is already configured for a parent of this path`),房間得改掛 `/root` | 初階/未驗(逃生艙;此坑實際踩到) | 「為什麼房間不能掛在 GameServer 底下?玩家那邊的 /root 是 Window 掛不了腳本,所以『玩家→伺服器』的意圖為什麼只能放 CardManager 上?」 |
| **意圖 / 結果是兩組 RPC**:`any_peer` 收意圖(伺服器驗證)、`authority` 廣播結果(玩家偽造被引擎丟棄)。舊架構 11 個 `any_peer + call_local` 是**雙端各自重放**,那跟權威模型從根本不相容。而且 UI 閘和伺服器閘**職責不同不是重複**:UI 閘=體驗(按了立刻有話說),伺服器閘=安全(繞過 UI 也擋得住) | 初階/未驗(逃生艙) | 「只留 UI 閘會怎樣?只留伺服器閘會怎樣?把 _net_end_turn 從 any_peer 改成 authority,改過的 client 直接呼叫它會發生什麼?」 |
| **入場券:先後手不能讓 client 自己報**:大廳發 token、房間驗 token,一張只能用一次。讓玩家在 req_join_room 裡自報側別 = 改過的 client 兩邊都報 "player" 搶先手 | 初階/未驗(逃生艙) | 「token 為什麼用完就 erase?猜中 token 的人能做什麼、不能做什麼?」 |
| **`--headless --editor --quit` 是假綠燈**:它回報零錯誤,但新腳本**不在任何場景裡就不會被編譯**——三個 `Cannot assign a new value to a constant` 全漏掉。真正抓到是「直接跑那支腳本」。與 §32 探針/回歸之分同族:綠燈要標注覆蓋範圍 | 初階/未驗(逃生艙自踩自修) | 「為什麼編輯器掃描沒抓到 net_client.gd 的錯?想確認一支不在場景裡的腳本編得過,最短的辦法是什麼?」 |
| **§23 的坑再踩一次**:經 preload 的 const 類別引用「寫」static 變數會被當成改常數擋下(讀和呼叫函式都行)。修法照舊:寫的動作收進該類別自己的 static 函式(`set_server_host` / `enter_room`) | 初階(2026-08-05 同一坑第二次踩;§23 已記過) | 「NET_MATCH.PORT 讀得到、reset() 叫得動,為什麼 NET_MATCH.room_port = x 編譯不過?為什麼把兩個欄位包成 enter_room() 比開兩個 setter 好?」 |
| **GDScript lambda 捕獲局部變數是「按值複製」**:`var paired := false` + `connect(func(): paired = true)` → 只改到 lambda 自己那份副本,外面永遠 false(白等 15 秒)。要跨 lambda 邊界回報狀態就得用成員變數 | 初階/未驗(逃生艙自踩自修) | 「哪些東西在 lambda 裡改了外面看得到、哪些看不到?為什麼成員變數可以?」 |
| **主動關閉連線也會觸發 server_disconnected**:配對成功後我們自己關大廳 peer,那條信號照樣發 → 若照「掉線 = 收攤」處理,`reset()` 會把剛拿到的側別/房間埠/入場券清成零。要用旗標區分「我關的」vs「真掉線」 | 初階/未驗(逃生艙自踩自修) | 「日誌上『配對成功』下一行就是『連線中斷』,為什麼?這類 bug 為什麼在單機測試裡看不到?」 |
| **三方拓撲下 client 的 rpc() 為什麼還到得了對手**:舊架構是 2 個 peer(對手就是 host),`rpc()` 送出去就到了。現在是 3 個 peer,client 的 `rpc()` 只送得到伺服器——**對手收得到是因為 `SceneMultiplayer.server_relay` 預設為 true**,伺服器自動轉發。所以那 12 個還沒改的行動 RPC 仍能運作(2026-08-05 實測:召喚兩邊都看得到、三方帳一致),它們缺的是**驗證**不是**送達** | 初階/未驗(逃生艙) | 「如果把 server_relay 關掉,現在哪些功能會立刻死、哪些還活著?為什麼『能送達』不等於『安全』?」 |
| **測試的判定不能看會被對手干擾的副作用**:第一版驗「越權意圖被擋」是看本機 `turn` 有沒有變——但對手可能剛好在那 1 秒內合法結束回合,turn 照樣跳 → 誤判 FAIL。改成讓伺服器在拒絕時 print,由 shell 層 grep**判決本身** | 初階/未驗(逃生艙自踩自修) | 「『伺服器擋住了』和『訊息根本沒送到』在玩家端的日誌上長得一樣——怎麼設計才分得出來?」 |

### 43. 中途離開對戰:模態窗與破壞性動作的攔截(2026-08-06;battle_ui / card_manager)

> 逃生艙代工。新增左上角「離開對戰」按鈕 + 全屏確認窗 + IDLE 時 ESC 開窗;
> 連線離開走既有 `_leave_online_match()`(關 peer,對手端由 2e 斷線流程收桌)。

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| **`_input` 比 GUI 早拿到事件**:Godot 的順序是 `_input` → Control(GUI) → `_unhandled_input`。CardManager 用的是 `_input`,所以蓋全屏的壓暗 ColorRect **攔不住 3D 拖曳與點擊**——模態窗要靠「狀態鎖」才擋得住,不是靠疊一層 Control(既有反制窗口靠 `ui_state = MENU_OPEN` 就是這個道理) | 初階/未驗(逃生艙) | 「反制窗口開著時為什麼玩家不能拖牌?如果 CardManager 改用 `_unhandled_input`,壓暗層會不會就擋得住了?那樣做有什麼副作用?」 |
| **狀態機鎖不夠用時,要加一個屬於這扇窗的旗標**:對手的 RPC(換回合)會呼叫 `_cancel_command()` 把 `ui_state` 打回 IDLE——窗還開著、鎖卻沒了。所以另立 `_leave_pending`,滑鼠事件先看旗標再看狀態 | 初階/未驗(逃生艙) | 「舉一個『窗開著但 ui_state 被別人改掉』的具體時序。為什麼加旗標比『不准 RPC 改狀態』好?」 |
| **收窗只能有一條路**:ESC/右鍵反悔走 `dismiss_leave_confirm()` → 內部呼叫和按鈕同一個 `_answer_leave(false)`,面板收起與旗標清除永遠成對。反過來 `show_game_over()` **只做視覺隱藏、不准走 dismiss**——那會 emit `leave_decided(false)` 把剛鎖上的 GAME_OVER 解開 | 初階/未驗(逃生艙) | 「若 ESC 只寫 `_leave_dim.visible = false`,下一步玩家會遇到什麼?為什麼勝負畫面那裡要走另一條?」 |
| **破壞性動作的 UI 慣例**:①入口離常用按鈕遠(離開在左上、結束回合在右下)②中間攔一道確認 ③安全選項排在前、危險選項染緋色——誤點第一顆的代價要是「什麼都沒發生」 | 初階/未驗(逃生艙) | 「如果把『確定離開』排在第一顆會怎樣?除了排序還有哪些手段降低誤觸代價?」 |
| **自動換行的 Label 一定要自己知道寬度上限**(2026-08-06 實際破圖):`autowrap_mode` 開了但沒設 `custom_minimum_size.x` → `reset_size()` 在 layout **之前**問它最小尺寸,它只能按「每個字一行」回報(45 個中文字 → 高 **1744px**),而 `PRESET_MODE_MINSIZE` 會把這個錯值**烤進 offsets**,之後換行算對了也回不來(面板變成一根跑到畫面外的黑柱)。父容器的 `custom_minimum_size` 救不了——那是**父的下限**,不是**子的上限** | 初階/未驗(逃生艙自踩自修) | 「為什麼反制窗口用同樣寫法卻一直沒炸?如果改成不設寬度、而是延後一幀再 `reset_size()`,行不行?兩種修法差在哪?」 |
| **破圖要先量數字再修**:肉眼「怪」和「錯在哪」是兩回事。`tests/screenshot_leave.gd` 印出 `panel size=(428,1744) pos=(362,-548)` 才一眼定位——高度四位數 = 最小尺寸算錯,負的 y = 被置中公式推到畫面外 | 初階/未驗(逃生艙) | 「只看截圖你會猜是什麼壞了?哪一個數字最快排除掉『壓暗層沒蓋滿』這個猜測?」 |
| **後果的文案屬於「知道模式的那一層」**:連線=認輸、單人/熱座=進度不留——判斷寫在 CardManager(`_leave_warning_text`),BattleUI 只收字串印出來。同 `open()` 的 `attack_note` / `skill_note`:規則在帳房、UI 只轉述 | 初階/未驗(逃生艙) | 「若讓 BattleUI 自己查 `NetMatch.is_online` 組文案,會違反這個專案的哪條慣例?之後多一種模式時哪邊要改?」 |

### 43. 秘術分派器補完:合約早就有的欄位,分派器沒讀(2026-08-07;battle_manager / card_manager / data/cards ×3)

> **背景**:Harvey 發現 768 顆圖示只用了 7 顆,想「反過來用卡圖聯想產卡」。
> 查證後發現瓶頸不是圖——是 `SkillData.Effect` 有 7 個值,從者技能接了 3 個、
> 秘術接了另外 3 個,兩邊都沒接滿。**這一節是逃生艙代工的**(「你幫我做吧」),
> Harvey 一行沒寫;§14 技能資料層維持 `初階/未驗`。

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| **「要不要選目標」是資料屬性,不是效果種類**:舊版 `_is_effect_spell` 是白名單 `effect in [DRAW, SCRY, DISCARD_DRAW]`,把「有沒有效果」和「要不要選目標」綁死了——HEAL 這種「有效果**又**要選目標」的卡根本表達不出來,會被白名單漏掉、當成傷害秘術拿 `power=0` 去打敵人。改問 `effect_target`(SELF=不用選 / ALLY / LANE_ENEMY),合約早就有這欄,只是沒人讀 | 初階/未驗(逃生艙) | 「白名單寫法下,一張『治療我方 3 點』的秘術會發生什麼?為什麼加 case 到白名單裡也治不好?」 |
| **改架構時,「沒寫 = 預設值」會突然變成有意義的**:三張抽濾卡的 `.tres` 從來沒寫 `effect_target`,一直正常——因為那時沒人讀它。改成由它決定流程後,它們會吃到 `@export` 的預設 `LANE_ENEMY` 而全部變成「要選目標」。**開始讀某個欄位 = 所有沒寫它的既有資料都被你改了語意** | 初階/未驗(逃生艙) | 「除了這三張,專案裡還有哪些欄位是『定義了但沒人讀』?哪天要讀它們時,該先做什麼?」 |
| **choke point 重用要連它的前提一起繼承**:把秘術接進 `_resolve_attack` 買到橫掃/貫穿/連擊/吸血只寫一份,但也繼承了它寫死的 `minion_attack = true` → 目標的**鐵壁**吃掉 1 點,火焰爆裂 3 傷變 2 傷,違反 §8「只減免**從者攻擊**」。修法是加 `minion_attack: bool = true` 預設參數(§24 的「預設參數擴充 API 不炸舊呼叫」),既有呼叫點一行沒改 | 初階/未驗(逃生艙;回歸測試抓到) | 「重用一個 choke point 之前,該問它哪一類問題?如果之後要做『無視鐵壁的從者攻擊技』,是再加參數還是加 Modifier 值?」 |
| **重複結算是隱形炸彈,而且錯誤訊息騙人**:`cast_arcana` 自己補了一次 `_check_death`,而 `_resolve_attack` 內部已經做過 → 死亡流程跑兩次 → 第二次 `side_of()` 查不到卡槽(第一次已清位)回空字串 → `sides[""]` 爆。引擎報的是「在 Dictionary 上用 String 存取」,**完全看不出根因是重複呼叫**。修法:`_check_death` 開頭加「查不到側別就 return」的門 | 初階/未驗(逃生艙;此坑實際踩到) | 「這個錯誤訊息為什麼沒指向真正的問題?下次看到『型別看起來很正常卻炸了』的訊息,第一個該懷疑什麼?」 |
| **測試 PASS 不等於沒錯**:`spell_smoke` 印 `PASS 15/15` 的同一份日誌裡有 SCRIPT ERROR——斷言只驗它問的那幾件事,引擎的抱怨在斷言之外。驗收要同時看**斷言結果**和 **SCRIPT ERROR 數**(§42 的假綠燈同族,不同變種) | 初階/未驗(逃生艙自踩自修) | 「哪些錯誤斷言天生抓不到?回歸測試腳本要加一條什麼樣的判定,才能把這類漏網之魚一起攔下?」 |
| **測試卡用 code 捏、不要塞進 data/cards**:`arcana_effects_test.gd` 驗的是**分派器接得對不對**(系統合約),不是某張卡的數值。假卡進 `data/cards/` 會汙染牌池抽牌與卡牌圖鑑 | 初階/未驗(逃生艙) | 「什麼情況下測試資料**應該**放進 data/?這批秘術真的要上線時,測試該改成驗什麼?」 |

### 44. 卡池對齊設計稿:護盾 + 戰吼上線(2026-08-10;skill_data / card_data / card / hero / battle_manager / card_manager / battle_ui / card_gallery / data/cards ×42)

> **背景**:Harvey 貼上 Pack01/02 共 42 張的卡牌設計稿,拍板「以清單為準,規格與資料層跟著擴」,
> 但保留 5 路線模型、這輪不做第二主動技。**這一節是逃生艙代工的**——訪談與架構是一起談的,
> `.gd` 與 42 張 `.tres` 由 AI 寫,Harvey 一行沒寫。決策與逐卡翻譯表見
> [card_rebalance_pack01_02.md](card_rebalance_pack01_02.md)。

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| **減免先做、吸收後做**:傷害管線是 夜幕減半 → 鐵壁 −1 → 護盾吸收 → 扣血。護盾排在減免之前也**不會報錯**,只會讓同一面盾少擋幾點(10 傷打「3 盾+夜幕+鐵壁」:盾先吸掉 3 血、盾後吸掉 1 血)。通則:**每層減免都要作用在「還沒被盾墊掉」的完整數字上** | 初階/未驗(逃生艙) | 「如果之後加一個『傷害 +2』的增益,它該插在這條線的哪裡?為什麼?」 |
| **「實際傷害」的定義會隨機制擴充而改變**:`_deal_damage` 的回傳值餵給吸血。護盾出現後,回傳的是**真正扣掉的 HP**、不含被盾吃掉的部分——盾不是血,砍在盾上沒有血可吸。加新機制時要回頭問「舊的回傳值語意還成立嗎」 | 初階/未驗(逃生艙) | 「5 點吸血打 3 盾的滿血目標,攻擊者回幾滴?若改成回傳『帳面傷害』,哪張卡會變得離譜?」 |
| **效果「做什麼」與「對誰做」是正交的兩件事**:原本非攻擊技的主體效果、攻擊技的附帶效果各寫一份 `match skill.effect`,加護盾時就得改兩處。收斂成 `_apply_skill_effect`(做什麼)+ `_effect_recipients`(對誰做)後,戰吼直接是第三個呼叫端,零額外分支 | 初階/未驗(逃生艙) | 「若要做『對己方全體上鍛強』(精英獸人的戰吼),要動哪一個函式、加幾行?」 |
| **戰吼復用 `SkillData` 而不是另開型別**:它要的欄位(effect / effect_target / amount / status / summon_card)一個不多一個不少;新型別只會多一份要同步維護的合約。差別只在「誰觸發」——主動技玩家點、戰吼自動跑,所以 kind/cost/anim 對戰吼沒意義 | 初階/未驗(逃生艙) | 「什麼情況下就**該**另開一個 BattlecryData?判準是什麼?」 |
| **choke point 的老招又用一次**:戰吼掛 `mark_summoned()`,因為它是所有召喚的必經之路(手動出牌/召喚技/召喚秘術/連線重放)。掛在出牌按鈕上的話,「死靈法師召出來的骷髏」永遠不會觸發自己的戰吼。**且戰吼要排在伏印之前**——否則暗影護甲的盾在最需要它的那一刻剛好還沒掛上 | 初階/未驗(逃生艙) | 「§7 伏印當初也挑這個點。除了召喚,專案裡還有哪些動作有類似的『必經之路』?」 |
| **規格模型不合時,翻譯而不是改模型**:設計稿是爐石式「自由選目標」寫的(嘲諷=逼你打我、飛行=越過嘲諷打後排),但本作是 5 路線直線攻擊——那些定義在這個棋盤上是空轉的。定一套固定翻譯規則(「最多 2 個」→ SPREAD_3、「所有單位」→ SPREAD_ALL、「隨機一個」→ 正對面)套 42 張,比重寫 §2/§4/§8 便宜兩個數量級 | 初階/未驗(逃生艙) | 「哪一條翻譯是**近似**而不是等價?若哪天真要改成自由選目標,最先會壞掉的是哪個函式?」 |
| **去隨機是連線對戰的隱性成本**:設計稿的「50% 機率復活」「隨機一個敵方單位」都被改掉——兩台要同步 RNG seed 才不會讓帳分岔,成本遠大於隨機買到的刺激感 | 初階/未驗(逃生艙) | 「如果真的想要隨機效果,在 host-client 架構下要怎麼做才不會分岔?」 |
| **卡面印得出「永遠不會發生的效果」**(Harvey 2026-08-10 自己抓到):蝙蝠 1/1/1 配吸血 → `heal()` 一律被 `mini(上限, 現血+量)` 夾成 0。設計稿讀起來很合理,是**數值與機制的交互**讓它變成 no-op。修法:保留味道、把血吸到有意義的地方(改回復己方英雄) | 初階(Harvey 主動發現,修法 AI 代工) | 「卡池裡還有哪些技能在特定數值下會變成 no-op?怎麼在加卡時就先聞出來?」 |
| **測試不該寫死平衡數值**:騎士 HP 6→5 讓 `spell_smoke` 三條斷言全紅,但驗的其實是「傷害管線的算術」,不是「騎士有幾滴血」。期望值改從 `CardData` 推,平衡再動也不會假紅 | 初階/未驗(逃生艙自踩自修) | 「哪些測試值**應該**寫死?『從資料推期望值』有沒有反而驗不到東西的風險?」 |
| **測試夾具會偷偷帶進自己的規則**:驗 SPREAD_ALL 時拿騎士當靶,它的【鐵壁】把 2 傷減成 1,兩條斷言假紅——機制是對的,是夾具多帶了一層減免。挑夾具要挑「沒有額外關鍵字」的白板 | 初階/未驗(逃生艙自踩自修) | 「除了鐵壁,還有哪些關鍵字會讓夾具汙染斷言?驗『純傷害』時該挑哪張卡?」 |

### 45. 高階卡池 92→120：跨上限費用、事件反應與動畫合約（2026-08-28;card_data / card / battle_manager / card_manager / data/cards ×28 / generated art）

> **背景**：自然魔力確認封頂 7，但卡池要出現 9–13 費終結卡；同時補齊從者、秘術、
> 瞬咒、伏印與靈裝，不使用領域。**這一節是逃生艙代工的**；卡表、程式、資料、
> 六組新角色動畫與 28 張卡圖均由 AI 產製，Harvey 負責拍板費用曲線與動畫原則。

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| **自然上限與可支付上限是兩個數字**：`mana_max` 封頂 7，只限制回合開始自動回滿；丟牌回魔直接加到 `mana`，所以棄 12 費卡可把 7 推到 13。若把 `mana` 也 clamp 7，高階卡會印得出來卻永遠不能用 | 初階/未驗(逃生艙) | 「13 費卡在滿魔時最少要棄幾費？為什麼不能把 MANA_CAP 同時套在暫時魔力？」 |
| **反應卡要用事件能力表，不是越寫越長的卡名 if**：瞬咒由 `special_id → 支援事件集合` 決定能否進 `arcana / summon / normal_attack / attack_skill / lethal / ward` 窗口；同一個 `quick_candidate` 入口讓所有事件沿用相同的手牌與費用判定 | 初階/未驗(逃生艙) | 「時隙屏障為什麼不該出現在秘術反制窗？絕對否決要新增一種事件時改哪一處？」 |
| **Resource 新欄位的預設值就是向後相容合約**：攻擊／生命加成、裝備關鍵字、普攻吸血、回合開始效果與指定復活血量全加在 `CardData`，舊 92 張不填就維持 0／false／1；執行期加成放在 `Card` 實例，避免污染所有同名卡共享的模板 | 初階/未驗(逃生艙) | 「為什麼不能直接把巨刃的 +5 寫進 target.data.atk？舊卡沒填 revive_hp 時會以幾血復活？」 |
| **網路可重放的反應先選確定性**：攻擊中的新瞬咒採雙端依同一份手牌／魔力自動挑第一張合法牌，不在動畫中途新增互動確認；這犧牲玩家選擇，但避免兩端在 0.35 秒演出窗口各自做不同決定。若未來要玩家選，必須由權威端廣播選擇結果 | 初階/未驗(逃生艙) | 「兩端各自跳詢問窗會怎樣分岔？要保留玩家選擇時，網路訊息應廣播『可選清單』還是『最後選中的卡』？」 |
| **從者動畫是資料合約，不是美術慣例**：所有從者必有普通攻擊 `Attack01`；只有具主動技能者才有獨立的 `Attack02`，兩者要在武器、動作輪廓與節奏上可辨識。驗收同時查 `.tres.active_skill.anim` 與實際 300×100 圖片，才能防止「資料寫 Attack02、檔案卻不存在」的假完成 | 初階/未驗(逃生艙) | 「無技能從者缺 Attack02 為什麼是正確的？只有檢查 PNG 存在、沒檢查 active_skill.anim 會漏掉哪種錯？」 |

### 46. 引擎 4.5 → 4.7 升級:UID 失效、@tool placeholder 與「保證不回傳 null」(2026-09-01;project.godot / 30 個 .tres+.tscn / player_hand / app_settings / tests/fix_stale_uids)

> **背景**:Harvey 把 Godot 換成 4.7.2-stable,要求「這邊也優化一下」,**逃生艙代工**。
> 結果 GDScript 那邊 76 支腳本零棄用、零 API 破壞——真正掉出來的三個問題**都是 4.5 時代就存在的舊帳**,
> 只是 4.7 把它們喊得比較大聲。編輯器匯入的警告/錯誤從 38 條降到 0 條。

| 觀念 | 等級 | 考題方向 |
|---|---|---|
| **失效 UID 是「現在能跑、未來會錯」的定時炸彈**:資產重匯入後拿到新 UID,但 `.tres`/`.tscn` 還留舊的。引擎退回字串路徑所以照樣載得到——這正是它長期沒被發現的原因。真正的危險是那組舊 ID 哪天被配給別的資源,就會**默默載到錯的東西**、而且不報錯 | 初階/未驗(逃生艙) | 「為什麼『能載到』反而讓這個問題更危險?如果 UID 對得上但**路徑**搬走了,引擎會怎麼處理?兩種壞法哪一種比較容易發現?」 |
| **要改引擎維護的檔案,用引擎的 API 改,不要手編**:版本標籤是 `ProjectSettings.set_setting()` + `save()` 寫的(diff 精準到只有那一行);UID 的正確值是 `ResourceLoader.get_resource_uid(path)` 問出來的,腳本只負責把答案抄回檔案——**全程沒有一個 UID 是我編的** | 初階/未驗(逃生艙) | 「如果我改用『load 進來再 ResourceSaver.save() 回去』來修 UID,對 .tres 和對 .tscn 的風險分別是什麼?為什麼 .tscn 比較危險?」 |
| **實測推翻了文件**:CLAUDE.md 原本寫「`config/features` 標籤留給編輯器同步」。跑了兩次 `--import`,project.godot 零 diff → **編輯器根本不會自己同步**。留著不管 = 標籤永遠停在 4.5 | 初階/未驗(逃生艙) | 「這條筆記當初怎麼會寫錯?要怎麼寫才能讓下一個人知道『這是推測』還是『這是實測過的』?」 |
| **`@tool` 腳本實體化非 `@tool` 場景 → placeholder instance**:編輯器裡拿到的節點只有 `Node3D` 那層是真的,腳本那層是空殼,碰 `setup()` / 自訂信號都會噴錯。而 `transform` 是 Node3D 的,所以扇形預覽其實一直是好的——壞的只有方法呼叫 | 初階/未驗(逃生艙) | 「`_arrange_fan()` 裡哪幾行在編輯器裡是安全的、哪幾行不是?判斷依據是什麼?如果改成讓 card.gd 也掛 `@tool`,會多出什麼風險?」 |
| **GDScript 的 lambda 是傳值捕獲**:寫 `func(): changed = true` 想在 lambda 裡改外層變數,外層**不會**變。修 UID 的腳本第一版就踩這個,改成「比較回傳值和原值」才對 | 初階/未驗(逃生艙自踩自修) | 「那要怎樣才能讓 lambda 真的改到外面的狀態?為什麼 GDScript 選傳值而不是傳參考?」 |
| **一個會回傳 null 的存取器 × 146 個沒做 null 檢查的呼叫點 = 埋在每支 `@tool` 腳本裡的地雷**:`AppSettings.current()` 在遊戲裡永遠不是 null(autoload 一定在),所以這顆雷躺了很久。修法選「讓 `current()` 保證不回傳 null」而不是「在呼叫點加 146 個 if」——**在源頭修一次 vs 在下游修 146 次** | 初階/未驗(逃生艙) | 「什麼情況下『回傳 null 讓呼叫端判斷』才是對的設計?這次為什麼不是?備援實例如果拿去存檔會發生什麼事?」 |
| **`static var` 裝 Node 會洩漏**:Node 不是 RefCounted,沒人 free 它。修完 null 那顆雷之後,匯入日誌從 0 條變成 2 條 leak 警告——**修一個問題製造另一個**。接 `root.tree_exiting` + `CONNECT_ONE_SHOT` 才歸零 | 初階/未驗(逃生艙自踩自修) | 「如果那個備援實例改成 `RefCounted` 而不是 `Node`,還需要手動 free 嗎?為什麼 `AppSettings` 不能改成 RefCounted?」 |
